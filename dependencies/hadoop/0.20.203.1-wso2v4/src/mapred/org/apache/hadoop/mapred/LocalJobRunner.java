  ...
  private class Job extends Thread
    ...
    public void run() {
      ...
      try {
        ...
        try {
          Path stagingDirectory = systemJobFile.getParent().getParent();  // sytemJobFile = {staging.dir.root}/{user}{jobid}/.staging/{job.attempt}/job.xml
          Path jobStagingDirectoryRoot = stagingDirectory.getParent();
          fs.delete(jobStagingDirectoryRoot, true);  // delete staging dir
          localFs.delete(localJobFile.getParent(), true);              // delete local copy (localRunner)
          // Cleanup distributed cache
          taskDistributedCacheManager.release();
          trackerDistributedCacheManager.purgeCache();

          localFs.delete(new Path(conf.get("mapred.local.dir")), true);  // delete local job setup directory root
        } catch (IOException e) {
