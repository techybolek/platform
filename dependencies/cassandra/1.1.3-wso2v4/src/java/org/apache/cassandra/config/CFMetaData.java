...
public final class CFMetaData
{
    ...
    //WSO2 to pass the resolved keyspace to update columnFamily

    public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def, String resolvedKeyspace) throws InvalidRequestException, ConfigurationException
    {
        ColumnFamilyType cfType = ColumnFamilyType.create(cf_def.column_type);
        if (cfType == null)
        {
        ...
        CFMetaData newCFMD = new CFMetaData(resolvedKeyspace,
                cf_def.name,
                cfType,
                TypeParser.parse(cf_def.comparator_type),
                cf_def.subcomparator_type == null ? null : TypeParser.parse(cf_def.subcomparator_type),
    ...
    // converts CFM to thrift CfDef
    //WSO2 to support tenant aware operations
    public org.apache.cassandra.thrift.CfDef toThrift(String keyspaceName)
    {
        org.apache.cassandra.thrift.CfDef def = new org.apache.cassandra.thrift.CfDef(keyspaceName, cfName);
        def.setId(cfId);
        def.setColumn_type(cfType.name());
        def.setComparator_type(comparator.toString());
        if (subcolumnComparator != null)
    ...
    static CFMetaData fromSchemaNoColumns(UntypedResultSet.Row result)
    {
        try
        {
            ...
            try
            {
                // try to identify if ColumnFamily Id is old style (before C* 1.1.1) and add old -> new mapping if so
                Schema.instance.addOldCfIdMapping(result.getInt("id"), cfm.cfId);
            }
            catch (Exception e)
            {
                // it was a UUID after all
            }
