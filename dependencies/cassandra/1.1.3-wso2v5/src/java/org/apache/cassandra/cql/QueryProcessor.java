...
public class QueryProcessor
{
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        //WSO2: batchupdate needs to get resolved keyspace
        String globalKeyspace = clientState.getResolvedKeyspace();
        ...
        for (UpdateStatement update : updateStatements)
        {
            ...
            {
                clientState.hasColumnFamilyAccess(keyspace, update.getColumnFamily(), Permission.WRITE, Action.UPDATE);
    ...
    throws  UnavailableException, InvalidRequestException, TimedOutException, SchemaDisagreementException
    {
        ...
        if (statement.type != StatementType.SELECT && StatementType.requiresKeyspace.contains(statement.type)){
            ...
            //WSO2: CQL query processing needs to retrun tenant aware KS.
            keyspace = clientState.getResolvedKeyspace();
        }
        ...
        switch (statement.type)
        {
            case SELECT:
                ...
                //WSO2:
                final String oldKeyspace = clientState.getResolvedKeyspace();
                ...
                clientState.hasColumnFamilyAccess(keyspace, select.getColumnFamily(), Permission.READ, Action.READ);
            ...
            case TRUNCATE:
                ...
                //WSO2:
                keyspace = columnFamily.left == null ? clientState.getResolvedKeyspace() : columnFamily.left;


                validateColumnFamily(keyspace, columnFamily.right);
                clientState.hasColumnFamilyAccess(keyspace, columnFamily.right, Permission.WRITE, Action.UPDATE);
            ...
            case DELETE:
                ...
                //WSO2:
                keyspace = delete.keyspace == null ? clientState.getResolvedKeyspace() : delete.keyspace;
            ...
            case DROP_INDEX:
                ...
                try
                {
                    ...
                    //WSO2
                    MigrationManager.announceColumnFamilyUpdate(dropIdx.generateCFMetadataUpdate(clientState.getResolvedKeyspace()));
            ...
            case ALTER_TABLE:
                ...
                clientState.hasColumnFamilyAccess(alterTable.columnFamily, Permission.WRITE,Action.UPDATE);
                validateSchemaAgreement();
