...
public class CassandraServer implements Cassandra.Iface
{
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        cState.hasColumnFamilyAccess(column_parent.column_family, Permission.READ, Action.READ);
        return multigetSliceInternal(cState.getResolvedKeyspace(), Collections.singletonList(key), column_parent, predicate, consistency_level).get(key);
    }
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        ClientState cState = state();
        cState.hasColumnFamilyAccess(column_parent.column_family, Permission.READ, Action.READ);
        return multigetSliceInternal(cState.getResolvedKeyspace(), keys, column_parent, predicate, consistency_level);
    }
    ...
    {
        ClientState cState = state();
        cState.hasColumnFamilyAccess(column_path.column_family, Permission.READ, Action.READ);
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        ClientState cState = state();
        cState.hasColumnFamilyAccess(column_parent.column_family, Permission.READ, Action.READ);
        Table table = Table.open(cState.getResolvedKeyspace());
    ...
    {
        ClientState cState = state();
        cState.hasColumnFamilyAccess(column_parent.column_family, Permission.WRITE, Action.ADD);

        CFMetaData metadata = ThriftValidation.validateColumnFamily(cState.getResolvedKeyspace(), column_parent.column_family, false);
        ThriftValidation.validateKey(metadata, key);
        ThriftValidation.validateColumnParent(metadata, column_parent);
        ...
        RowMutation rm = new RowMutation(cState.getResolvedKeyspace(), key);
        try
        {
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException
    {
        ...
        for (Map.Entry<ByteBuffer, Map<String, List<Mutation>>> mutationEntry: mutation_map.entrySet())
        {
            ...
            for (Map.Entry<String, List<Mutation>> columnFamilyMutations : columnFamilyToMutations.entrySet())
            {
                ...
                {
                    cState.hasColumnFamilyAccess(cfName, Permission.WRITE, Action.UPDATE);
    ...
    {
        ClientState cState = state();
        cState.hasColumnFamilyAccess(column_path.column_family, Permission.WRITE, Action.DELETE);

        CFMetaData metadata = ThriftValidation.validateColumnFamily(cState.getResolvedKeyspace(), column_path.column_family, isCommutativeOp);
        ThriftValidation.validateKey(metadata, key);
    ...
    {
        ThriftValidation.validateConsistencyLevel(state().getResolvedKeyspace(), consistency_level, RequestType.WRITE);
    ...
    {
        state().hasKeyspaceSchemaAccess(Permission.READ, Action.READ);

        KSMetaData ksm = Schema.instance.getTableDefinition(state().resolveKeyspace(table));
        if (ksm == null){
            //logger.info("describe_keyspace does not exist: " + state().resolveKeyspace(table) +
            //        "   TID: " + state().getTenantUser());
            throw new NotFoundException();
        }

        //return ksm.toThrift().setName(state().getDomainNameLessKeyspace(table));
        return ksm.toThrift(state().getDomainNameLessKeyspace(table));
    }
    ...
    throws InvalidRequestException, UnavailableException, TimedOutException, TException
    {
        ...
        ClientState cState = state();
        String keyspace = cState.getResolvedKeyspace();
        cState.hasColumnFamilyAccess(column_family, Permission.READ, Action.READ);
    ...
    public List<KsDef> describe_keyspaces() throws TException, InvalidRequestException
    {
        ...
        for (String ks : keyspaces)
        {
            try
            {
                //ksset.add(describe_keyspace(ks).setName(state().getDomainNameLessKeyspace(ks)));
    ...
    throws TException, InvalidRequestException
    {
        ...
        List<Token> tokens = StorageService.instance.getSplits(state().getResolvedKeyspace(), cfName, new Range<Token>(tf.fromString(start_token), tf.fromString(end_token)), keys_per_split);
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        state().hasColumnFamilySchemaAccess(Permission.WRITE, Action.ADD);
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        ClientState cState = state();
        cState.hasColumnFamilySchemaAccess(Permission.WRITE, Action.DELETE);
        validateSchemaAgreement();

        try
        {
            MigrationManager.announceColumnFamilyDrop(cState.getResolvedKeyspace(), column_family);
            return Schema.instance.getVersion().toString();
        }
        catch (ConfigurationException e)
        {
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        ThriftValidation.validateKeyspaceNotSystem(state().resolveKeyspace(ks_def.name));
        state().hasKeyspaceSchemaAccess(Permission.WRITE, Action.ADD);
        validateSchemaAgreement();
        ThriftValidation.validateKeyspaceNotYetExisting(state().resolveKeyspace(ks_def.name));
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        ThriftValidation.validateKeyspaceNotSystem(state().resolveKeyspace(keyspace));
        state().hasKeyspaceSchemaAccess(Permission.WRITE, Action.DELETE);
        validateSchemaAgreement();

        try
        {
            MigrationManager.announceKeyspaceDrop(state().resolveKeyspace(keyspace));
            return Schema.instance.getVersion().toString();
        }
        catch (ConfigurationException e)
        {
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        state().hasKeyspaceSchemaAccess(Permission.WRITE, Action.UPDATE);
    ...
    throws InvalidRequestException, SchemaDisagreementException, TException
    {
        ...
        state().hasColumnFamilySchemaAccess(Permission.WRITE, Action.UPDATE);
        ...
        try
        {
            ...
            //CFMetaData cfm = CFMetaData.fromThrift(cf_def);
            //WSO2 set the resolved ks to CF def
            CFMetaData cfm = CFMetaData.fromThrift(cf_def,state().resolveKeyspace(cf_def.keyspace));
            cfm.addDefaultIndexNames();
    ...
    {
        ClientState cState = state();
        logger.debug("truncating {} in {}", cfname, cState.getResolvedKeyspace());
        cState.hasColumnFamilyAccess(cfname, Permission.WRITE, Action.UPDATE);
        try
        {
            schedule(DatabaseDescriptor.getRpcTimeout());
