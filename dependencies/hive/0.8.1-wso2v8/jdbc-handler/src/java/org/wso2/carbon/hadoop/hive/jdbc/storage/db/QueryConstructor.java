...
public class QueryConstructor {
    ...
    /**
     * SELECT * FROM (SELECT *, ROW_NUMBER() OVER (ORDER BY column_name) AS Row FROM table_name) AS tmp WHERE Row > 10 AND Row < 20
     * @param dbProperties
     * @param split
     * @return
     */
    private String getQueryForMsSql(DatabaseProperties dbProperties, JDBCSplit split) {
        ...
        query.append("SELECT *, ROW_NUMBER() OVER (ORDER BY ").append(fieldNames[0]).append(")");
        query.append(" AS Row FROM ").append(dbProperties.getTableName());
...
/*        query.append(" AS ").append(dbProperties.getTableName()); //in hsqldb this is necessary*/
    ...
    private String getQueryForOracle(DatabaseProperties dbProperties, JDBCSplit split){
        String[] fieldNames = dbProperties.getFieldsNames();
        StringBuilder allFieldsNames = new StringBuilder();
        StringBuilder query = new StringBuilder();

        for (int i = 0; i < fieldNames.length; i++) {
            allFieldsNames.append(fieldNames[i]);
            if (i != fieldNames.length - 1) {
                allFieldsNames.append(", ");
            }
        }

        query.append("SELECT ").append(allFieldsNames).append(" FROM ( SELECT ROW_NUMBER() OVER( ORDER BY ");
        query.append(fieldNames[0]);
        query.append(" ) LINENUM, ").append(allFieldsNames);
        query.append(" FROM ");
        query.append(dbProperties.getTableName());
