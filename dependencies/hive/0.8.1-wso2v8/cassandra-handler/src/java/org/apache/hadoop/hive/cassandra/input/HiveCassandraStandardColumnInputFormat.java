...
import org.apache.hadoop.hive.cassandra.CassandraStorageHandler;
...
public class HiveCassandraStandardColumnInputFormat extends InputFormat<BytesWritable, MapWritable>
    ...
    private boolean isMapDataTypePresent;
    private ColumnFamilyInputFormat cfif;
    ...
    public RecordReader<BytesWritable, MapWritable> getRecordReader(InputSplit split,
        ...
        isMapDataTypePresent = AbstractColumnSerDe.isMapDataTypePresent(columns);
        ...
        org.apache.hadoop.mapreduce.InputSplit cfSplit = cassandraSplit.getSplit();
        ...
        if (isTransposed || readColIDs.size() == columns.size() || readColIDs.size() == 0) {
            ...
            if (isMapDataTypePresent) {
                // If map data type present fetch all columns from the row. Needs to be used with caution with wide rows.
                SliceRange range = new SliceRange();
                range.setStart(ByteBufferUtil.bytes(""));
                range.setFinish(ByteBufferUtil.bytes(""));
                predicate.setSlice_range(range);
            } else {
        ...
        try {
            ...
            if (isTransposed && tac.getConfiguration().getBoolean(AbstractColumnSerDe.CASSANDRA_ENABLE_WIDEROW_ITERATOR, true)) {
                ...
                rr = new org.apache.hadoop.hive.cassandra.input.CassandraHiveRecordReader(new ColumnFamilyRowRecordReader(), isTransposed);
            }
...
//    IndexExpression expression  = new IndexExpression(ByteBuffer.wrap("fromTimeStamp".getBytes()),
//            IndexOperator.GT, ByteBuffer.wrap(jobConf.get("hive.marker.markerName.from.time").getBytes()));
//     indexExpressions.add(expression);
//    ConfigHelper.setInputRange(jobConf, indexExpressions);
        ...
        for (int i = 0; i < splits.size(); ++i) {
            HiveCassandraStandardSplit csplit = new HiveCassandraStandardSplit(
                    (InputSplit) splits.get(i), cassandraColumnMapping, tablePaths[0]);
    ...
    public List<org.apache.hadoop.mapreduce.InputSplit> getSplits(JobContext context)
            throws IOException {
        if (null == cfif) {
            if (Boolean.parseBoolean(context.getConfiguration().get(HiveConf.ConfVars.HIVE_INCREMENTAL_PROCESS_ENABLE.toString()))) {
                ...
                boolean isCFIncremental = false;
                if (null != hiveIncrementalCassandraTables) {
                ...
                if (isCFIncremental){
                    cfif = new IncrementalColumnFamilyInputFormat();
                }
                else cfif = new ColumnFamilyInputFormat();

                return cfif.getSplits(context);
            } else {
                cfif = new ColumnFamilyInputFormat();
                return cfif.getSplits(context);
            }
        } else {
            return cfif.getSplits(context);
    ...
    public org.apache.hadoop.mapreduce.RecordReader<BytesWritable, MapWritable> createRecordReader(
        ...
        if (isTransposed && tac.getConfiguration().getBoolean(AbstractColumnSerDe.CASSANDRA_ENABLE_WIDEROW_ITERATOR, true)) {
            ...
            return new CassandraHiveRecordReader(new ColumnFamilyRowRecordReader(), isTransposed);
        }
    }
