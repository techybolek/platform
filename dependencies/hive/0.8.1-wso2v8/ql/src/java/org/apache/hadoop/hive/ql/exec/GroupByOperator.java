...
import org.apache.hadoop.hive.ql.udf.generic.IncrementalBuffer;
...
public class GroupByOperator extends Operator<GroupByDesc> implements
    ...
    private void estimateRowSize() throws HiveException {
        ...
        ArrayList<ExprNodeDesc> keysOnlyInGroupBy = conf.getKeysOnlyInGroupBy();
    ...
    */
     ...
     *
     * @param keys The keys in the record
    ...
    protected void forward(Object[] keys, AggregationBuffer[] aggs)
        ...
        //this keys stores the group by stuff...
        for (int i = 0; i < keys.length; i++) {
            forwardCache[i] = keys[i];
        }

        String groupByString = "";
        if(null != conf.getKeysOnlyInGroupBy()){
        int maxIndex = conf.getKeysOnlyInGroupBy().size() - 1;

        int iteration = 0;
        for (Object aKey : keys) {
            if (iteration <= maxIndex) {
                groupByString += String.valueOf(aKey) + "_";
            } else {
                break;
            }
            iteration++;
        ...
        for (int i = 0; i < aggs.length; i++) {
            AggregationBuffer aggBuffer = aggs[i];
            if (aggBuffer instanceof IncrementalBuffer) {
                ((IncrementalBuffer) aggBuffer).setRowKey(groupByString);
            }
