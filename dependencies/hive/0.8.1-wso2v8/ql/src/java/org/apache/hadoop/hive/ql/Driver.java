...
public class Driver implements CommandProcessor {
  ...
  public Driver() {
    conf = HiveContext.getCurrentContext().getConf();  
    if (conf == null && SessionState.get() != null) {
  ...
  public int execute() throws CommandNeedRetryException {
    ...
    try {
      ...
      }                                                   //hive_2012-12-08_20-24-18_126_5322268490656038147

      String hiveScratchDir = ctx.getLocalScratchDir(false);
      try {
        if (hiveScratchDir != null && ctx.getResDir() == null) {
            Path jobScratchDir = new Path(hiveScratchDir);

            if (LOG.isDebugEnabled()) {
                LOG.debug("Cleaning scratchDir : " + hiveScratchDir);
            }

            FileSystem fileSystem = jobScratchDir.getFileSystem(conf);
