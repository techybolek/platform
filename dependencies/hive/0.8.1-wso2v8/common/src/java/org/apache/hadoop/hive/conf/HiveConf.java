...
public class HiveConf extends Configuration {
  ...
  public static enum ConfVars {
    ...
    CURRENTTENANT("hive.current.tenant", -1234),
    ...
    // Is it required to retain Hive query logs (Hive history logs) after the session tear down?
    HIVERETAINHISTORYLOGS("hive.retain.query.logs", false),
...
/*    private void processFileUrls() {
        String hadoopFsURL = this.get(ConfVars.HADOOPFS.varname);

        String carbonHome = System.getProperty("CARBON_HOME");

        if (hadoopFsURL != null && hadoopFsURL.contains("${CARBON_HOME}")) {
            hadoopFsURL.replace("${CARBON_HOME}", carbonHome);
        }

        this.set(ConfVars.HADOOPFS.varname, hadoopFsURL);

        String metaStoreWarehouseURL = this.get(ConfVars.METASTOREWAREHOUSE.varname);

        if (metaStoreWarehouseURL != null && metaStoreWarehouseURL.contains("${CARBON_HOME}")) {
            metaStoreWarehouseURL.replace("${CARBON_HOME}", carbonHome);
        }

        this.set(ConfVars.METASTOREWAREHOUSE.varname, metaStoreWarehouseURL);
    }*/
