    ...
    protected void passToNextStates(StateEvent eventBundle) {
        ...
        if (distributedProcessing) {
            if (currentState == higherState) {
                ((OrPatternState) state).getPartnerState().getInnerHandlerProcessor().removeFromCurrentEvents(eventBundle);
            } else  {
                ((OrPatternState) state).getPartnerState().getInnerHandlerProcessor().removeFromNextEvents(eventBundle);
            }
        }
