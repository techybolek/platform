...
public class StateParser {
    ...
    public static List<PatternState> convertToPatternStateList(List<State> stateList) {
        ...
        for (int i = 0, stateListSize = stateList.size(); i < stateListSize; i++) {
            ...
            if (state instanceof AndState) {
                patternState = (new AndPatternState(state.getStateNumber(), state.getBasicStream()));
            } else if (state instanceof OrState) {
                patternState = (new OrPatternState(state.getStateNumber(), state.getBasicStream()));
            } else if (state instanceof CountState) {
                patternState = (new CountPatternState(state.getStateNumber(), state.getBasicStream(), ((CountState) state).getMin(), ((CountState) state).getMax()));
            } else {
                patternState = (new PatternState(state.getStateNumber(), state.getBasicStream()));
            }
    ...
    private static List<State> identifyStates(PatternElement patternElement, List<State> stateList,
        ...
        if (patternElement instanceof BasicStream) {
            stateList.add(new State(stateNumber.getNumber(), ((BasicStream) patternElement)));
            ...
            if (((LogicalElement) patternElement).getType() == LogicalElement.Type.OR) {
                stateList.add(new OrState(stateNumber.getNumber(), ((LogicalElement) patternElement).getBasicStream1(), stateNumber.getNumber() + 1));
                addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
                stateList.add(new OrState(stateNumber.getNumber(), ((LogicalElement) patternElement).getBasicStream2(), stateNumber.getNumber() - 1));
                //addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
            } else {//AND
                stateList.add(new AndState(stateNumber.getNumber(), ((LogicalElement) patternElement).getBasicStream1(), stateNumber.getNumber() + 1));
                addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
                stateList.add(new AndState(stateNumber.getNumber(), ((LogicalElement) patternElement).getBasicStream2(), stateNumber.getNumber() - 1));
                //addStateAsNext(stateList, stateNumber, perv);
                stateNumber.increment();
            }
            perv.clear();
            ...
            stateList.add(new CountState(stateNumber.getNumber(), ((CountElement) patternElement).getBasicStream(), ((CountElement) patternElement).getMinCount(), ((CountElement) patternElement).getMaxCount()));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            perv.clear();
            perv.add(stateNumber.getNumber() - 1);
...
//            }
    ...
    public static List<SequenceState> convertToSequenceStateList(List<State> stateList) {
        ...
        for (int i = 0, stateListSize = stateList.size(); i < stateListSize; i++) {
            ...
            if (state instanceof OrState) {
                sequenceState = (new OrSequenceState(state.getStateNumber(), state.getBasicStream()));
            } else if (state instanceof CountState) {
                sequenceState = (new CountSequenceState(state.getStateNumber(), state.getBasicStream(), ((CountState) state).getMin(), ((CountState) state).getMax()));
            } else {
                sequenceState = (new SequenceState(state.getStateNumber(), state.getBasicStream()));
            }
    ...
    private static List<State> identifyStates(SequenceElement sequenceElement,
        ...
        if (sequenceElement instanceof BasicStream) {
            stateList.add(new State(stateNumber.getNumber(), ((BasicStream) sequenceElement)));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            perv.clear();
            perv.add(stateNumber.getNumber() - 1);
        } else if (sequenceElement instanceof OrElement) {
            stateList.add(new OrState(stateNumber.getNumber(), ((OrElement) sequenceElement).getBasicStream1(), stateNumber.getNumber() + 1));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            stateList.add(new OrState(stateNumber.getNumber(), ((OrElement) sequenceElement).getBasicStream2(), stateNumber.getNumber() - 1));
            //addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            ...
            stateList.add(new CountState(stateNumber.getNumber(), ((RegexElement) sequenceElement).getBasicStream(), ((RegexElement) sequenceElement).getMinCount(), ((RegexElement) sequenceElement).getMaxCount()));
            addStateAsNext(stateList, stateNumber, perv);
            stateNumber.increment();
            perv.clear();
            perv.add(stateNumber.getNumber() - 1);
