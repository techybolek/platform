...
import org.wso2.siddhi.core.util.collection.queue.scheduler.timestamp.ISchedulerTimestampQueue;
import org.wso2.siddhi.core.util.collection.queue.scheduler.timestamp.SchedulerTimestampQueue;
import org.wso2.siddhi.core.util.collection.queue.scheduler.timestamp.SchedulerTimestampQueueGrid;
...
public class TimeWindowProcessor extends WindowProcessor implements RunnableWindowProcessor {
    ...
    private ISchedulerTimestampQueue<StreamEvent> window;

    @Override
    public void setParameters(Object[] parameters) {
        if (parameters[0] instanceof Integer) {
            timeToKeep = (Integer) parameters[0];
        } else if (parameters[0] instanceof Long) {
            timeToKeep = (Long) parameters[0];
        } else if (parameters[0] instanceof IntConstant) {
    ...
    public Iterator<StreamEvent> iterator(String predicate) {
        if (siddhiContext.isDistributedProcessing()) {
            return ((SchedulerTimestampQueueGrid<StreamEvent>) window).iterator(predicate);
        } else {
            return window.iterator();
        }
    }
    ...
    protected void initWindow() {
        if (siddhiContext.isDistributedProcessing()) {
            window = new SchedulerTimestampQueueGrid<StreamEvent>(elementId, this, siddhiContext, async);
        } else {
            window = new SchedulerTimestampQueue<StreamEvent>(this);
        }
    }
