...
public class QueryManager {
    ...
    public QueryManager(Query query, ConcurrentMap<String, StreamDefinition> streamDefinitionMap,
        ...
        initQuery(queryEventStreamList, query);
    ...
    private void initQuery(List<QueryEventStream> queryEventStreamList, Query query) {
        ...
        if (attributeList.size() == 0) {
            if (query.getInputStream() instanceof SingleStream && queryEventStreamList.size() == 1) {
                ...
                for (Attribute attribute : queryEventStreamList.get(0).getStreamDefinition().getAttributeList()) {
    ...
    private void validateOutput(List<QueryEventStream> queryEventStreamList) {
        ...
        for (OutputAttribute outputAttribute : outputAttributeList) {
            if (outputAttribute instanceof SimpleAttribute) {
                ExpressionValidator.validate(((SimpleAttribute) outputAttribute).getExpression(), queryEventStreamList, null);
            }

            if (outputAttribute instanceof ComplexAttribute) {
                for (Expression expression : ((ComplexAttribute) outputAttribute).getExpressions()) {
                    ExpressionValidator.validate(expression, queryEventStreamList, null);
                }
            }
        }
