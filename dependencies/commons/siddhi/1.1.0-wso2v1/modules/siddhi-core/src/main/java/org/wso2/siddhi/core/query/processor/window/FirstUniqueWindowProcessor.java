    ...
    public void setParameters(Object[] parameters) {
        ...
        for (int i = 0, parametersLength = parameters.length; i < parametersLength; i++) {
            ...
            if (parm instanceof String) {
                uniqueAttributeNames[i] = (String) parm;
            } else {
    ...
    protected void processEvent(InListEvent listEvent) {
        ...
        try {
            ...
            if (tempMapSize == 1) {
                ...
                InListEvent inListEvents = new InListEvent(tempMap.size());

                for (java.util.Map.Entry<String, InEvent> entry : tempMap.entrySet()) {
                    StreamEvent oldEvent = map.put(entry.getKey(),  new RemoveEvent(entry.getValue(), Long.MAX_VALUE));
                    if (oldEvent == null) {
                        inListEvents.addEvent(entry.getValue());
                    }
                }
                if (inListEvents.getActiveEvents() == 1) {
                    nextProcessor.process(inListEvents.getEvent(0));
                } else if (inListEvents.getActiveEvents() > 1) {
                    nextProcessor.process(inListEvents);
                }
            }
        } finally {
            releaseLock();
