...
public class DistributedGroupByAggregationAttributeProcessor extends AbstractAggregationAttributeProcessor implements GroupByAttributeProcessor {
    ...
    private volatile Map<String, Aggregator> tempAggregatorMap = new HashMap<String, Aggregator>();

    public DistributedGroupByAggregationAttributeProcessor(Expression[] expressions, List<QueryEventStream> queryEventStreamList, AttributeAggregatorFactory attributeAggregatorFactory, String elementId, SiddhiContext siddhiContext) {
    ...
    public synchronized Object process(AtomicEvent event, String key) {
        ...
        if (!lockedAcquired) {
            ...
            try {
                currentAggregator = distributedAggregatorMap.getAsync(key).get();
            ...
            currentAggregator = tempAggregatorMap.get(key);
            if (currentAggregator == null) {
                try {
                    currentAggregator = distributedAggregatorMap.getAsync(key).get();
                    if (currentAggregator == null) {
                    ...
                    tempAggregatorMap.put(key, currentAggregator);
                } catch (Exception e) {
                    log.error(e);
                }
            }
        ...
        if (!lockedAcquired) {
            distributedAggregatorMap.putAndUnlock(key, currentAggregator);
        }
        return value;
    }
    ...
    public Object[] currentState() {
        Map<String, Aggregator> tempMap = new HashMap<String, Aggregator>();
        for (Map.Entry<String, Aggregator> entry : distributedAggregatorMap.entrySet()) {
    ...
    public void restoreState(Object[] objects) {
        distributedAggregatorMap.putAll((Map<String, Aggregator>) objects[0]);
    }
    ...
    public synchronized void unlock() {
        if (lockedAcquired) {
            for (Map.Entry<String, Aggregator> entry : tempAggregatorMap.entrySet()) {
