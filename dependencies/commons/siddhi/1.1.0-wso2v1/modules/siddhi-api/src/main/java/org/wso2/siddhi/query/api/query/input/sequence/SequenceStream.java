...
public class SequenceStream implements Stream, SequenceElement {
    ...
    private HashSet<String> collectStreamIds(SequenceElement sequenceElement,
        ...
        if (sequenceElement instanceof SequenceStream) {
            ...
            collectStreamIds(((OrElement) sequenceElement).getBasicStream1(), streamIds);
            collectStreamIds(((OrElement) sequenceElement).getBasicStream2(), streamIds);
        } else if (sequenceElement instanceof RegexElement) {
            collectStreamIds(((RegexElement) sequenceElement).getBasicStream(), streamIds);
    ...
    public List<QueryEventStream> constructEventStreamList(SequenceElement sequenceElement,
        ...
        if (sequenceElement instanceof BasicStream) {
            ...
            constructEventStreamList(((OrElement) sequenceElement).getBasicStream1(), streamDefinitionMap, queryEventStreams);
            constructEventStreamList(((OrElement) sequenceElement).getBasicStream2(), streamDefinitionMap, queryEventStreams);
        } else if (sequenceElement instanceof RegexElement) {
            constructEventStreamList(((RegexElement) sequenceElement).getBasicStream(), streamDefinitionMap, queryEventStreams);
        } else if (sequenceElement instanceof NextElement) {
