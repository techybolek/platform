...
public class PatternStream implements Stream, PatternElement {
    ...
    private HashSet<String> collectStreamIds(PatternElement patternElement,
        ...
        if (patternElement instanceof PatternStream) {
            ...
            collectStreamIds(((LogicalElement) patternElement).getBasicStream1(), streamIds);
            collectStreamIds(((LogicalElement) patternElement).getBasicStream2(), streamIds);
        } else if (patternElement instanceof CountElement) {
            collectStreamIds(((CountElement) patternElement).getBasicStream(), streamIds);
    ...
    public List<QueryEventStream> constructEventStreamList(PatternElement patternElement,
        ...
        if (patternElement instanceof BasicStream) {
            ...
            constructEventStreamList(((LogicalElement) patternElement).getBasicStream1(), streamDefinitionMap, queryEventStreams);
            constructEventStreamList(((LogicalElement) patternElement).getBasicStream2(), streamDefinitionMap, queryEventStreams);
        } else if (patternElement instanceof CountElement) {
            constructEventStreamList(((CountElement) patternElement).getBasicStream(), streamDefinitionMap, queryEventStreams);
        } else if (patternElement instanceof FollowedByElement) {
