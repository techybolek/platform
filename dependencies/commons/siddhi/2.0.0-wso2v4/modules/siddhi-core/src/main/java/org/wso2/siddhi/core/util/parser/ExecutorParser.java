...
import org.wso2.siddhi.core.executor.expression.AbstractGenericExpressionExecutor;
...
import org.wso2.siddhi.core.extension.holder.ConditionExtensionHolder;
...
public class ExecutorParser {
    ...
    public static ConditionExecutor parseCondition(Condition condition,
        ...
        if (condition instanceof AndCondition) {
            ...
            AbstractGenericConditionExecutor conditionExecutor = (AbstractGenericConditionExecutor) SiddhiClassLoader.loadExtensionImplementation((ConditionExtension) condition, ConditionExtensionHolder.getInstance(siddhiContext));
            conditionExecutor.setSiddhiContext(siddhiContext);
            conditionExecutor.setExpressionExecutors(expressionExecutors);
    ...
    public static ExpressionExecutor parseExpression(Expression expression,
        ...
        if (expression instanceof Constant) {
            ...
            AbstractGenericExpressionExecutor expressionExecutor = (AbstractGenericExpressionExecutor) SiddhiClassLoader.loadExtensionImplementation((ExpressionExtension) expression, ExpressionExtensionHolder.getInstance(siddhiContext));
            expressionExecutor.setSiddhiContext(siddhiContext);
            expressionExecutor.setInnerExpressionExecutors(innerExpressionExecutors);
