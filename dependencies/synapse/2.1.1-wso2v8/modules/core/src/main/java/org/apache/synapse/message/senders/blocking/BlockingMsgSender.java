...
public class BlockingMsgSender {
    ...
    public MessageContext send(Endpoint endpoint, MessageContext synapseInMsgCtx)
        ...
        AxisService anonymousService =
                AnonymousServiceFactory.getAnonymousService(null,
                                                            configurationContext.getAxisConfiguration(), endpointDefinition.isAddressingOn() | endpointDefinition.isReliableMessagingOn(), endpointDefinition.isReliableMessagingOn(), endpointDefinition.isSecurityOn(), false);
        ...
        try {
            if (isOutOnly) {
                ...
                synapseInMsgCtx.setProperty(NhttpConstants.HTTP_SC, result.getProperty(HTTPConstants.MC_HTTP_STATUS_CODE));
                ...
                if (ex instanceof AxisFault) {
                    ...
                    if (faultMC != null) {
                        synapseInMsgCtx.setProperty(NhttpConstants.HTTP_SC,
                                                    faultMC.getProperty(HTTPConstants.MC_HTTP_STATUS_CODE));
    ...
    private org.apache.axis2.context.MessageContext sendReceive(org.apache.axis2.context.MessageContext axisOutMsgCtx, Options clientOptions,
                                       AxisService anonymousService, ServiceContext serviceCtx)
            throws AxisFault {
        ...
        axisOutMsgCtx.setAxisMessage(axisAnonymousOperation.getMessage(WSDLConstants.MESSAGE_LABEL_OUT_VALUE));
        operationClient.execute(true);
        org.apache.axis2.context.MessageContext resultCtx = operationClient.getMessageContext(WSDLConstants.MESSAGE_LABEL_IN_VALUE);
        axisOutMsgCtx.getTransportOut().getSender().cleanup(axisOutMsgCtx);
    ...
    private boolean isOutOnly(MessageContext messageIn,
        ...
        return "true".equals(messageIn.getProperty(SynapseConstants.OUT_ONLY)) || axis2Ctx.getOperationContext() != null && WSDL2Constants.MEP_URI_IN_ONLY.equals(axis2Ctx.getOperationContext().getAxisOperation().getMessageExchangePattern());
    }
