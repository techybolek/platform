<!--
 ...
 ~ KIND, either express or implied.  See the License for theresources
...
<%
    ...
    try {
        ...
        HDFSAdminClient client = new HDFSAdminClient(config.getServletContext(), session);
//         synapseRegistryRoot = client.getProperty(synapseRegistryMetaDataRootPath, "SYNAPSE_REGISTRY_ROOT");
        if (client.getCurrentUserFSObjects(rootPath) != null) {
...
<script type="text/javascript" src="../hdfsmgt/js/hdfs_util.js"></script>
...
%>
            <a onclick="loadSubTree('<%=rootPath%>', '<%=rootName%>', '<%=textBoxId%>', '<%=(hideResources? "true" : "false")%>');">
                <img style="margin-right: 5px;" id="plus_<%=rootName%>" src="../hdfsmgt/images/icon-tree-plus.jpg"/>
                <img style="display:none; margin-right: 5px;" id="minus_<%=rootName%>" src="../hdfsmgt/images/icon-tree-minus.jpg"/></a>
            <a onclick="pickPath('<%=rootPath%>', '<%=textBoxId%>', '<%=rootName%>');"><img style="margin-right: 2px;"
                                                                                            src="../hdfsmgt/images/folder.png"/><%=rootPath%></a>
        </div>
...
<%
        ...
        if (!expansionPath.equals("")) {
            ...
            if (!path.equals("/")) {
                ...
                try {
               // 	FolderInformation[] folders = client.getCurrentUserFSObjects(path);
              //      HDFSTreeEntryBean bean = client.getHDFSTreeEntry(path);
                //    isCollection = bean.isCollection();//bean.getCollection();
                } catch (Exception ignored) {
