...
function getParentPath(srcPath){
		return srcPath.substr(0, srcPath.lastIndexOf("/"));
}
...
function hideOthers(id, type) {
    ...
    symLinkPanel.style.display = "none";
...
function renameItem(srcPath, resourceEditDivID, isFolder) {
		sessionAwareFunction(function() {
         var reason = "";
        document.getElementById(resourceEditDivID).value = ltrim(document.getElementById(resourceEditDivID).value);
        reason += validateEmpty(resourceEditDivID, org_wso2_carbon_hdfs_mgt_ui_jsi18n["name"]);
       if (reason == "") {
        ...
        if (reason != "") {
        	...
        	var parentPath = getParentPath(srcPath);
            var newName = parentPath +"/"+ document.getElementById(resourceEditDivID).value;
            ...
            {
                method:'post',
                parameters: {srcPath: srcPath, dstPath: newName, isFolder: isFolder},

                onSuccess: function() {
                	 CARBON.showInfoDialog(successMsg);
                  //  displaySuccessMessage(resourcePath, org_wso2_carbon_hdfs_mgt_ui_jsi18n["renamed"]);
                     fillContentSection(parentPath);
                },
...
function createFolder(srcPath, createDivID)
{
	sessionAwareFunction(function() {
	var newName = document.getElementById(createDivID).value;
	var newFolder = srcPath+'/'+newName; 
	new Ajax.Request('../hdfsmgt/create_folder_ajaxprocessor.jsp',
	         {
	                ...
	                onSuccess: function() {
	                	 CARBON.showInfoDialog(org_wso2_carbon_hdfs_mgt_ui_jsi18n["folder.create.successful"]);
	                    fillContentSection(srcPath);
	                },
		    ...
		    new Ajax.Request('../hdfsmgt/delete_folder_ajaxprocessor.jsp',
	       	         {
		    				method:'post',
	       	                ...
	       	                onSuccess: function() {
	       	                	CARBON.showInfoDialog(org_wso2_carbon_hdfs_mgt_ui_jsi18n["folder.delete.successful"]);
	       	                    fillContentSection(getParentPath(folderToDelete));
	       	                },
	...
	new Ajax.Request('../hdfsmgt/delete_file_ajaxprocessor.jsp',
  	         {
   				method:'post',
  	                ...
  	                onSuccess: function() {
  	                	CARBON.showInfoDialog(org_wso2_carbon_hdfs_mgt_ui_jsi18n["file.delete.successful"]);
  	                    fillContentSection(getParentPath(filePath));
  	                },
...
function fillFileUploadDetails() {
...
function submitUploadUpdatedContentForm() {
    ...
    if (rForm.upload.value.length == 0) {
        reason += validateEmpty(rForm.upload, org_wso2_carbon_hdfs_mgt_ui_jsi18n["file.path"]);
    }
...
//    sessionAwareFunction(function() {
		var rForm = document.forms["fileUploadForm"];
        var reason = "";

        if (fileName.length == 0) {
            reason +=  "File path not filled" + "<br />";
        }
...
function fillContentSection(path, pageNumber, viewMode, consumerID, targetDivID) {
        ...
        var random = getRandom();
        new Ajax.Updater('contentDiv', '../hdfsmgt/standard_view_ajaxprocessor.jsp', { method: 'get',onComplete:loadData, parameters: {path: path, requestedPage: pageNumber, mode: 'standard',resourceViewMode:viewMode,resourcePathConsumer:consumerID,targetDivID:targetDivID,random:random} });
...
function pickPath(path, textBoxId, reference) {
    if (!textBoxId || textBoxId == 'null') {
        ...
        window.location = "../hdfsmgt/hdfs_dashboard.jsp?region=region3&item=resource_browser_menu&viewType=std&path=" + path.replace(/&/g, "%26");
        return;
    }
...
function loadFromPath() {
    ...
    window.location = "../hdfsmgt/hdfs_dashboard.jsp?region=region3&item=resource_browser_menu&viewType=std&path=" + pickedPath.value.replace(/^\s\s*/, '').replace(/\s\s*$/, '').replace(/&/g, "%26");
}
