...
public class CoordinationConfiguration {
    ...
    public static final String PROTOCOL_HANDLER_USERNAME_ALIAS = "HumanTask.Coordination.ProtocolHandler.Username";
    public static final String PROTOCOL_HANDLER_PASSWORD_ALIAS = "HumanTask.Coordination.ProtocolHandler.Userpassword";
    ...
    private CoordinationConfiguration() {
        ...
        this.enable = tHtCoordinationConfig.getEnable();
        this.registrationServiceEnabled = tHtCoordinationConfig.getEnableRegistrationService();
        if (tHtCoordinationConfig.getHumanTaskEngineProtocolHandler() != null
                && tHtCoordinationConfig.getHumanTaskEngineProtocolHandler().getAuthentication() != null) {
            // Reading secured password
            getAuthenticationConfig(htCoordinationConfigFile, tHtCoordinationConfig.getHumanTaskEngineProtocolHandler().getAuthentication());
        } else {
    ...
    private void getAuthenticationConfig(File file, TTaskAuthenticationConfig authentication) {
        ...
        if (secretResolver != null && secretResolver.isInitialized()
            ...
            log.warn("Using unsecured plan text username. It is recommended to configure secure vault for username in."
        ...
        if (secretResolver != null && secretResolver.isInitialized()
            ...
            log.warn("Using unsecured plan text password. It is recommended to configure secure vault for password in."
                    + HUMAN_TASK_COORDINATION_CONFIG_FILE);
        }
    }
