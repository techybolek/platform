...
import org.wso2.carbon.humantask.coordination.module.utils.ServerUtils;
...
public class HTCoordinationContextHandler extends AbstractHandler implements Handler {
    ...
    public InvocationResponse invoke(MessageContext messageContext) throws AxisFault {
        ...
        try {
            humanTaskProtocolHandlerServiceURL = ServerUtils.getTaskProtocolHandlerURL(messageContext.getConfigurationContext());
        ...
        // We are OK to invoke Registration service
        try {
            OMElement response = InvokeRegistrationService(identifier, humanTaskProtocolHandlerServiceURL, registrationService);
    ...
    private OMElement InvokeRegistrationService(String identifier, String taskProtocolHandlerServiceURL, String registrationService) throws AxisFault {
        ...
        if (registrationService.equals(tenantDomain)) {
            ...
            CarbonUtils.setBasicAccessSecurityHeaders(serverConfig.getRegServiceAuthUsername(), serverConfig.getRegServiceAuthPassword(), serviceClient);
        } else {
            if (log.isDebugEnabled()) {
            ...
            CarbonUtils.setBasicAccessSecurityHeaders(
                    serverConfig.getRegServiceAuthUsername() + "@" + tenantDomain,
                    serverConfig.getRegServiceAuthPassword() + "@" + tenantDomain,
                    serviceClient);
        }
