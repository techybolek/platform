...
<script type="text/javascript">
    ...
    function ValidateTextForm(form) {
        if (IsEmpty(form.Name)) {
            ...
            CARBON.showWarningDialog('<fmt:message key="provider.url.cannot.be.empty"/>')
    ...
    function addServiceParams() {
        ...
        addServiceParameter("java.naming.provider.url", document.getElementById('provider_url').value);
        addServiceParameter("store.jms.destination", document.getElementById('jms_destination').value);
        addServiceParameter("store.jms.connection.factory", document.getElementById('jms_connection_factory').value);
            ...
            <div id="_advancedForm" style="display:none">
                ...
                <table class="normal-nopadding">
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="jms_password" name="jms_password"
                    ...
                    <tr>
                        <td><fmt:message key="store.jms.cache.connection"/></td>
                        <td>

                            <select id="jms_cache_connection">
                                <%
                                    if ((null != messageStore) && (messageStore.getParams().get("store.jms.cache.connection") != null) && (messageStore.getParams().get("store.jms.cache.connection").equals("true"))) {
                                %>
