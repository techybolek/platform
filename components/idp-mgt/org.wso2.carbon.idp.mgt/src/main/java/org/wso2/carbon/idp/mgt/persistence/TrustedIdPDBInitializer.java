 ...
 * SQL scripts for creating the tables inside $CARBON_HOME/dbscripts/relying-party directory.
 */
public class TrustedIdPDBInitializer {

    private static Log log = LogFactory.getLog(TrustedIdPDBInitializer.class);
    private static final String DB_CHECK_SQL = "select * from IDN_BASE_TABLE";
    private DataSource dataSource;
    private String delimiter = ";";
    Statement statement;

    TrustedIdPDBInitializer(DataSource dataSource) {
        this.dataSource = dataSource;
    }

    void createTrustedIdPDB() throws IdentityProviderMgtException {
        if (!isDatabaseStructureCreated()) {
            ...
            try {
                ...
                executeSQLScript();
                conn.commit();
                log.debug("Trusted IdP metadata store are created successfully.");
            } catch (SQLException e) {
                String msg = "Failed to create database tables for Trusted IdP meta-data store";
                log.fatal(msg, e);
                throw new IdentityProviderMgtException(msg);
            } catch (IOException e) {
                String msg = "Failed to close input stream after reading Trusted IdP meta-data store DB scripts";
                log.error(msg, e);
            } finally {
                try {
                    if (conn != null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Trusted IdP metadata database already exists. Not creating a new database");
            }
        }
    }
    ...
    private void executeSQLScript() throws IdentityProviderMgtException, SQLException, IOException {

        String databaseType = TrustedIdPDBInitializer.getDatabaseType(dataSource.getConnection());
        ...
        try {
            ...
            String errorMsg = "Error occurred while executing SQL script for creating Trusted IdP metadata database";
    ...
    public static String getDatabaseType(Connection conn) throws IdentityProviderMgtException {
        String type = null;
        try {
            ...
            log.fatal(msg, e);
            throw new IdentityProviderMgtException(msg, e);
        }
        return type;
    }
    ...
    protected String getDbScriptLocation(String databaseType) {
        ...
        return carbonHome +
                "/dbscripts/service-provider/" + scriptName;
    }
     ...
     * @throws Exception
     */
    private void executeSQL(String sql) throws IdentityProviderMgtException {
        ...
        try {
            ...
            if (e.getSQLState().equals("X0Y32") || e.getSQLState().equals("42710")) {
                ...
                throw new IdentityProviderMgtException("Error occurred while executing : " + sql, e);
            }
        } finally {
            if (resultSet != null) {
                try {
