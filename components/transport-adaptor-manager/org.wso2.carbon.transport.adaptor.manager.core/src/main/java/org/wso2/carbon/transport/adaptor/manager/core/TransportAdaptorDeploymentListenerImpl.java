...
import org.wso2.carbon.transport.adaptor.core.TransportAdaptorDeploymentListener;
...
public class TransportAdaptorDeploymentListenerImpl implements TransportAdaptorDeploymentListener {

    private static final Log log = LogFactory.getLog(TransportAdaptorDeploymentListenerImpl.class);

    public void tryTransportAdaptorDeployment(){
        ...
        try {
            carbonTransportAdaptorManagerService.deployTransportAdaptorConfigurationFiles();
        } catch (TransportAdaptorManagerConfigurationException e) {
            log.error("Exception occurred while deploying the Transport Adaptor configuration files");
        }
    }
