               ...
               success:function (msg) {
                   if (msg != null) {
                       ...
                       if (supportedTransportAdaptorType == 'inout' && commonTransportProperties != undefined) {
                           tableRow.innerHTML = '<td colspan="2" ><b><fmt:message key='transport.common'/></b></td> ';
                       }
                       else if ((inputTransportProperties != undefined) || (outputTransportProperties != undefined || (commonTransportProperties != undefined))) {
                           tableRow.innerHTML = '<td colspan="2" ><b><fmt:message key='transport.all.properties'/></b></td> ';
                       }
                       ...
                       if (commonTransportProperties != undefined) {
                           ...
                           $.each(commonTransportProperties, function (index,
                               ...
                               loadTransportProperties('common', commonTransportProperty, transportInputTable, transportCommonPropertyLoop, commonProperty, commonRequiredProperty, 'commonFields', supportedTransportAdaptorType)
                       ...
                       if (supportedTransportAdaptorType == 'inout') {
                           var tableRow = transportInputTable.insertRow(transportInputTable.rows.length);
                           tableRow.innerHTML = '<td colspan="2" class="middle-header"><fmt:message key='transport.input'/> <input type="checkbox" id="inputCheckbox" onclick="enableMyInput(this)" align="bottom"/></td> ';
                       ...
                       if (inputTransportProperties != undefined) {
                           ...
                           if (supportedTransportAdaptorType == 'inout') {
                               ...
                               tableRowHeading.innerHTML = '<td colspan="2"><b><fmt:message key='transport.input.properties'/></b></td> ';
                           }

                           $.each(inputTransportProperties, function (index,
                                                                      inputTransportProperty) {

                               loadTransportProperties('input', inputTransportProperty, transportInputTable, transportInputPropertyLoop, inputProperty, inputRequiredProperty, 'inputFields', supportedTransportAdaptorType)
                       ...
                       if (supportedTransportAdaptorType == 'inout') {
                           var tableRow = transportInputTable.insertRow(transportInputTable.rows.length);
                           tableRow.innerHTML = '<td colspan="2" class="middle-header"><fmt:message key='transport.output'/>  <input type="checkbox" id="outputCheckbox" onclick="enableMyInput(this)" align="bottom"/> </td>';
                       ...
                       if (outputTransportProperties != undefined) {
                           ...
                           if (supportedTransportAdaptorType == 'inout') {
                               var tableRowHeading = transportInputTable.insertRow(transportInputTable.rows.length);
                               tableRowHeading.innerHTML = '<td colspan="2"><b><fmt:message key='transport.output.properties'/></b></td> ';
                           }
                           $.each(outputTransportProperties, function (index,
                                                                       outputTransportProperty) {
                               loadTransportProperties('output', outputTransportProperty, transportInputTable, transportOutputPropertyLoop, outputProperty, outputRequiredProperty, 'outputFields', supportedTransportAdaptorType)
...
function loadTransportProperties(propertyType, transportProperty, transportInputTable,
    ...
    if (transportProperty.localOptions == '') {
        ...
        if (propertyType != 'common' && supportedAdaptorType == 'inout') {
        ...
        var option = '';
        $.each(transportProperty.localOptions, function (index, localOption) {
        ...
        if (propertyType != 'common' && supportedAdaptorType == 'inout') {
            ...
            if (hint != undefined) {
                inputField.innerHTML = '<div class="' + classType + '"> <select   id="' + requiredElementId + transportPropertyLoop + '"disabled=disabled"' + '" name="' + transportProperty.localKey + '">' + option + '</select><br/> <div class="sectionHelp">' + hint + '</div></div>';
            }
            else {
                inputField.innerHTML = '<div class="' + classType + '"> <select  id="' + requiredElementId + transportPropertyLoop + '"disabled=disabled"' + '" name="' + transportProperty.localKey + '"  />' + option + ' </div>';
            }
        }
        else {
            if (hint != undefined) {
...
function enableMyInput(obj) {
    ...
    if (jQuery(obj).attr('id') == "outputCheckbox") {
        if ((jQuery(obj).is(':checked'))) {
            ...
            jQuery('.outputFields').find('select').attr('disabled', false)
        }
        else {
            jQuery('.outputFields').find('input', 'text').val("")
            jQuery('.outputFields').find('input').attr('disabled', 'disabled')
            jQuery('.outputFields').find('select').attr('disabled', 'disabled')
        }
    }
        ...
        if ((jQuery(obj).is(':checked'))) {
            ...
            jQuery('.inputFields').find('select').attr('disabled', false)
        }
        else {
            jQuery('.inputFields').find('input', 'text').val("")
            jQuery('.inputFields').find('input').attr('disabled', 'disabled')
            jQuery('.inputFields').find('select').attr('disabled', 'disabled')
        }
    }
...
<tr>
    ...
    <td><select name="transportTypeFilter"
                onchange="showTransportProperties()" id="transportTypeFilter">
        <%
...
<tr>
    <td colspan="2"><b>
        <fmt:message key="transport.common"/> </b>
    </td>
</tr>
...
<tr>
    <td colspan="2"><b>
        <fmt:message key="transport.all.properties"/> </b>
    </td>
</tr>
...
<tr>
    ...
    <td>
        <div class=commonFields>
            <%
                if (commonTransportProperties[index].getOptions()[0] != null) {
            %>
            <select name="<%=commonTransportProperties[index].getKey()%>"
                    id="<%=propertyId%><%=index%>">

                <%
                    for (String property : commonTransportProperties[index].getOptions()) {
                        if (property.equals(commonTransportProperties[index].getDefaultValue())) {
                %>
...
<tr>
    <td colspan="2" class="middle-header">
        <fmt:message key="transport.input"/>
...
<tr>
    <td colspan="2"><b>
        <fmt:message key="transport.input.properties"/></b>
    </td>
</tr>
...
<tr>
    <td colspan="2" class="middle-header">
        <fmt:message key="transport.output"/>
...
<tr>
    <td colspan="2"><b>
        <fmt:message key="transport.output.properties"/> </b>
    </td>
</tr>
<%
    }
...
<tr>
    ...
    <td>
        <div class=outputFields>
            <%
                if (supportedTransportAdaptorType.equals("inout")) {
                    if (outTransportProperties[index].getOptions()[0] != null) {
            %>

            <select name="<%=outTransportProperties[index].getKey()%>"
                    id="<%=propertyId%><%=index%>">

                <%
                    for (String property : outTransportProperties[index].getOptions()) {
                        if (property.equals(outTransportProperties[index].getDefaultValue())) {
                %>
                <option selected="selected"><%=property%>
                </option>
                <% } else { %>
