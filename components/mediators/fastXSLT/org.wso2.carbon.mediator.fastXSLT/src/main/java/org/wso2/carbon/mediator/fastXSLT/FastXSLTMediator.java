...
public class FastXSLTMediator extends AbstractMediator implements ManagedLifecycle {
    ...
    public boolean mediate(MessageContext context) {
        ...
        try {
        	...
        	String SOAPEnvBegin =context.isSOAP11() ? SOAP_11_ENV_ST:SOAP_12_ENV_ST;
        	String SOAPEnvEnd =context.isSOAP11() ? SOAP_ENV_END_11:SOAP_ENV_END_12;
        	Pipe pipe= (Pipe) axis2MC.getProperty(PassThroughConstants.PASS_THROUGH_PIPE);
            ...
            IOUtils.write(SOAPEnvBegin.getBytes(), _transformedOutMessageNew);
            IOUtils.write(_transformedOutMessage.toByteArray(), _transformedOutMessageNew);
            IOUtils.write(SOAPEnvEnd.getBytes(), _transformedOutMessageNew);
        	...
        	boolean fullLenthDone =false;
         	if((_transformedOutMessage.toByteArray().length+SOAPEnvBegin.length()+SOAPEnvEnd.length())>bufferSizeSupport){
        	...
        	if (!fullLenthDone && Boolean.TRUE.equals(axis2MC.getProperty(PassThroughConstants.MESSAGE_BUILDER_INVOKED))) {
            	...
            	IOUtils.write(SOAPEnvBegin.getBytes(),msgContextOutStream);
                IOUtils.write(_transformedOutMessage.toByteArray(),msgContextOutStream);
                IOUtils.write(SOAPEnvEnd.getBytes(),msgContextOutStream);
