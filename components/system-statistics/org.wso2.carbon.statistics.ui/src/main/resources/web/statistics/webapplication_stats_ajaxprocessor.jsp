...
<%@ page import="org.wso2.carbon.statistics.ui.WebappStatisticAdminClient" %>
...
<%
    ...
    WebappStatisticAdminClient adminClient = new WebappStatisticAdminClient(cookie,backendServerURL,configContext,request.getLocale()) ;


    try {
        adminClient.setWebappStatistics(webAppName);
    } catch (Exception e) {
...
<fmt:bundle basename="org.wso2.carbon.statistics.ui.i18n.Resources">
    <table class="styledLeft" id="serviceStatsTable" width="100%" style="margin-left: 0px;">
        ...
        <tr class="tableOddRow">
            ...
            <td><%=adminClient.getRequestCount()%>
            </td>
        </tr>
        <tr class="tableEvenRow">
            <td><fmt:message key="response.count"/></td>
            <td><%=adminClient.getResponseCount() %>
            </td>
        </tr>
        <tr class="tableOddRow">
            <td><fmt:message key="fault.count"/></td>
            <td><%=adminClient.getFaultCount() %>
            </td>
        </tr>
        <tr class="tableEvenRow">
            <td><fmt:message key="maximum.response.time"/></td>
            <td><%=adminClient.getMaximumResponseTime() %> ms</td>
        </tr>
        <tr class="tableOddRow">
            <td><fmt:message key="minimum.response.time"/></td>
            <td>
                <%if (adminClient.getMinimumResponseTime() == 0 &&
                        adminClient.getResponseCount() > 0) { %>
                &lt; 1.00 ms
                <%} else {%>
                <%=adminClient.getMinimumResponseTime() %> ms
                <%}%>
            </td>
        </tr>
        <tr class="tableEvenRow">
            <td><fmt:message key="average.response.time"/></td>
            <td><%=((float) Math.round(adminClient.getAverageResponseTime() * 1000)) / 1000%> ms</td>
        </tr>
        <tr>
            <td colspan="2">&nbsp;</td>
        </tr>
        ...
        <script type="text/javascript">
            ...
            try {
                graphAvgResponse.add(<%= adminClient.getAverageResponseTime() %>);
