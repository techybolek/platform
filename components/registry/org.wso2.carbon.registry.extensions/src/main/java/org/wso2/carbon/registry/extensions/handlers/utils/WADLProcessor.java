...
public class WADLProcessor {
    ...
    public String addWadlToRegistry(RequestContext requestContext, Resource resource,
        ...
        if(!skipValidation)  {
            ...
            try{
                ...
                bufferedWriter.write(wadlContent);
            ...
            validateWADL("file://" + tempFile.getAbsolutePath());
        }
        ...
        String actualPath = getChrootedWadlLocation(requestContext.getRegistryContext()) +
                namespaceSegment + RegistryUtils.getResourceName(resourcePath);
        ...
        if(getCreateService()){
            OMElement serviceElement = getServiceElement(requestContext,
                    wadlName.substring(0, wadlName.indexOf(".")), wadlNamespace, actualPath);
            CommonUtil.addService(serviceElement, requestContext);
        }

        resolveImports(wadlElement, actualPath, null);
    ...
    public String importWADLToRegistry(RequestContext requestContext, String commonLocation, boolean skipValidation)
        ...
        String wadlName = RegistryUtils.getResourceName(uri);
        String wadlNamespace = wadlElement.getNamespace().getNamespaceURI();
        ...
        if(commonLocation != null){
            ...
            actualPath = RegistryConstants.GOVERNANCE_REGISTRY_BASE_PATH +
                    commonWADLLocation + namespaceSegment + RegistryUtils.getResourceName(uri);
        }
        requestContext.setResourcePath(new ResourcePath(actualPath));
        registry.put(actualPath, resource);
        ...
        resolveImports(wadlElement, actualPath, wadlBaseUri);

        return actualPath;
    }

    private void resolveImports(OMElement wadlElement,
                                String resourcePath, String wadlBaseUri) throws RegistryException {
        String wadlNamespace = wadlElement.getNamespace().getNamespaceURI();
        ...
        if(grammarsElement != null){
            ...
            while (grammarElements.hasNext()){
                ...
                if(importUrl.endsWith(".xsd")) {
                    ...
                    addDependency(resourcePath, schemaPath);
                }
            }
        }
    }
    ...
    private void validateWADL(String uri) throws RegistryException {
        ...
        try {
            ...
            String msg = "Untrusted WADL URL found";
            throw new RegistryException(msg, e);
        }
    }
