...
/**
 * This class retrieves the requested resource according to the path parameters passed with the request.
 */

@Path("/resource")
public class Resources extends RestSuper{
	
	protected Log log = LogFactory.getLog(Resources.class);
	/**
		...
		try{
			...
			if (exist){
				...
				if (resource instanceof Collection){
					return Response.ok().entity(resource.getContent()).type("application/json").build();
				} else{
					//get the content of the resource as a stream
					String fileName = resourcePath.substring(resourcePath.lastIndexOf("/")+1);
					return Response.ok(resource.getContentStream()).type(resource.getMediaType()).header("Content-Disposition", "attachment; filename="+fileName).build();
				}
			} else{
	 ...
	 * @param username - username of the enduser
	 * @param tenantID - tenant ID of the enduser belongs to username
	 * @param type - mediatype of the resource will be in this format(<mediatype>;<encoding type>)
	 * @param input - resource content sent as input stream
	...
	public Response createResource(@PathParam("path") List<PathSegment> path,InputStream input,
			@HeaderParam("Content-Type")String type,@QueryParam("username")String username,
		...
		try {
			...
			if (exist) {
				...
				//extract the <mediatype> from the <mediatype>;<encoding format>
				if(type.indexOf(";") > 0){ 
					type = type.substring(0, type.indexOf(";"));
				}
				//check for collection media type 
				if(type.contains("application/atomcoll+xml")){
					resource = userRegistry.newCollection();
