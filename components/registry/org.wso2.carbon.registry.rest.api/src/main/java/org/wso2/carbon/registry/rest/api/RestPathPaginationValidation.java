...
/**
 * this class validates whether the resource path is null or not. If null return a BAD Request error.
 * If not null, check whether user specifies the page start and size (no of records to be fetched out) 
 * less than 0, returns a HTTP BAD request error code.  
 */
public class RestPathPaginationValidation {
	
	protected static Log log = LogFactory.getLog(RestPathPaginationValidation.class);
	
	protected static int validate(String resourcePath,int start,int size){
		//null check for resource path and invalid argument check for pagination params. 
		if(resourcePath == null || resourcePath.equals("")|| start < 0 || size < 0){
