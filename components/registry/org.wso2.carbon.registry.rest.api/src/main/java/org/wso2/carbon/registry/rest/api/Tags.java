...
public class Tags extends PaginationCalculation<Tag>{
	...
	protected Log log = LogFactory.getLog(Tags.class);
	...
	public Response getTagsOnAResource(@QueryParam("path")String resourcePath,@QueryParam("start") int start,
			@QueryParam("size")int size,@QueryParam("username")String username,
		...
		try {
			...
			if (exist){
				...
				//return getTagsPaginationIndex(start,size,resourcePath);
			} else{
	...
	@Produces("application/json")
	public Response setTagsOnAResource(@QueryParam("path")String resourcePath,
			TagModel tags,@QueryParam("username")String username,@QueryParam("tenantid")String tenantID){
		...
		try{
			boolean exist = userRegistry.resourceExists(resourcePath);
			if(exist){
				String[] tagsOnResource = tags.getTags();
				for(int i = 0; i < tagsOnResource.length; i++){
					userRegistry.applyTag(resourcePath, tagsOnResource[i]);
				}
	...
	private Response getAllTags()
	{
		if(setTagSearchQuery())
		{
			...
			try {
				...
				collection = userRegistry.executeQuery(RegistryConstants.CONFIG_REGISTRY_BASE_PATH+
				        RegistryConstants.QUERIES_COLLECTION_PATH + "/tags",Collections.<String, String>emptyMap());
				//create the tagList, extract the tag name from collection and add it to tagList if
	...
	/**
	 * custom query for all the tags search is set as a resource and saved at the config registry.space
	 */
	private boolean setTagSearchQuery(){
		...
		try {
			if (!userRegistry.resourceExists(tagsQueryPath)) {
				...
				resource.setContent("SELECT RT.REG_TAG_ID FROM REG_RESOURCE_TAG RT ORDER BY " +"RT.REG_TAG_ID");
				resource.setMediaType(RegistryConstants.SQL_QUERY_MEDIA_TYPE);
				resource.addProperty(RegistryConstants.RESULT_TYPE_PROPERTY_NAME,RegistryConstants.TAGS_RESULT_TYPE);
				userRegistry.put(tagsQueryPath, resource);
			}
			return true;
		} catch (RegistryException e){ 
			log.error(e.getCause(),e);
	...
	protected Tag[] getResult(){
		Tag[] result = null;
		try{
			result = userRegistry.getTags(path);
		}catch(RegistryException e){ 
	...
	protected Response display(Tag[] tagArr,int begin, int end){
		...
		TagModel tagModel = new TagModel(tagNames);
