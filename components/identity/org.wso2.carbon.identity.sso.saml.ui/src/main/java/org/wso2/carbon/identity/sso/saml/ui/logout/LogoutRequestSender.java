        ...
        public void run() {
            ...
            try {
                    ...
                    if (response != null && SAMLSSOUIUtil.isHttpSuccessStatusCode(statusCode)) {
                        ...
                        try {
                            ...
                            log.info("Sending single log out request again with retry count " +
                                     ...
                                     SAMLSSOUIUtil.getSingleLogoutRetryInterval() + " milli seconds to " +
                ...
                if (!isSuccessfullyLogout) {
                    log.error("Single logout failed after retrying " + SAMLSSOUIUtil.getSingleLogoutRetryCount() +
                              " times with time interval " + SAMLSSOUIUtil.getSingleLogoutRetryInterval() + " in milli seconds.");
                }
