...
<%
    ...
    String filter = request.getParameter(SAMLSSOProviderConstants.ISSUER_LIST_FILTER);
    String cookie;
    String serverURL;
    ...
    if (isPaginatedString != null && isPaginatedString.equals("true")) {
        spConfigClient = (SAMLSSOConfigServiceClient) session.getAttribute(SAMLSSOProviderConstants.CONFIG_CLIENT);
    }
    ...
    if (filter == null || filter.trim().length() == 0) {
            filter = (String) session.getAttribute(SAMLSSOProviderConstants.ISSUER_LIST_FILTER);
            if (filter == null || filter.trim().length() == 0) {
                filter = "*";
            }
    }
    filter = filter.trim();
    session.setAttribute(SAMLSSOProviderConstants.ISSUER_LIST_FILTER, filter);
    ...
    try {
        ...
        if (serviceProviderInfoDTO.getServiceProviders() != null) {
            SAMLSSOServiceProviderDTO[] filteredProviders = Util.doFilter(filter, serviceProviderInfoDTO.getServiceProviders() ) ;
            numberOfPages = (int) Math.ceil((double) filteredProviders.length / itemsPerPageInt);
            SAMLSSOServiceProviderDTO[] paginatedServiceProviders = Util.doPaging(pageNumberInt, filteredProviders);
            Collections.addAll(providers, paginatedServiceProviders);
        }
        if (reload != null && "true".equals(reload)) {
            configPath = spConfigClient.reloadSAML2FedaretedIdpConfig(true);
        } else {
            configPath = spConfigClient.reloadSAML2FedaretedIdpConfig(false);
        }
    } catch (AxisFault e) {
...
<div id="middle">
    ...
    <div id="workArea">
        ...
        <%
            if (configPath != null) {
        %>
        <form method="POST" action="manage_service_providers.jsp?reload=true">
            <table class="styledLeft" width="100%" id="FederatedIdps">
                <thead>
                <tr>
                    <th><fmt:message key="saml2.federation"/></th>
                </tr>
                </thead>
                <tbody>

                <tr>
                    <td width="100%"><%=configPath%>&nbsp;&nbsp;<input class="button" type="submit"
                                                                       value="<fmt:message key="saml2.federation.reload"/>"/>
                    </td>
                </tr>
                </tbody>
            </table>
            ...
            <table class="styledLeft noBorders">
                ...
                <tr>
                    <td class="leftCol-big" style="padding-right: 0 !important;"><fmt:message
                            key="list.issuer"/></td>
                    <td>
                        <input type="text" name="<%=SAMLSSOProviderConstants.ISSUER_LIST_FILTER%>"
                               value="<%=filter%>"/>
