...
public class EntitlementMediator extends AbstractMediator implements ManagedLifecycle {
    ...
    public boolean mediate(MessageContext synCtx) {
        ...
        try {
            ...
            int envLength=env.length;
            Attribute[] tempArr=new Attribute[envLength+3];
            ...
            for(int i=0;i<envLength;i++){
                tempArr[3+i]= new Attribute("urn:oasis:names:tc:xacml:3.0:attribute-category:environment", "urn:oasis:names:tc:xacml:1.0:environment:environment-id", ProxyConstants.DEFAULT_DATA_TYPE, env[i]);
            }
