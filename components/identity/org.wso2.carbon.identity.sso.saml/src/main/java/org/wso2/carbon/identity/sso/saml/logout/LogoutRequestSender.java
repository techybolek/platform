    ...
    private class LogoutReqSenderTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                for (int currentRetryCount = 0; currentRetryCount < Util.getSingleLogoutRetryCount(); currentRetryCount++) {
                    int statusCode = 0;
                    ...
                    if (response != null && Util.isHttpSuccessStatusCode(statusCode)) {
                        ...
                        try {
                            synchronized (Thread.currentThread()) {
                                Thread.currentThread().wait(Util.getSingleLogoutRetryInterval());
                            }
                            log.info("Sending single log out request again with retry count " +
                                     (currentRetryCount + 1) + " after waiting for " +
                                     Util.getSingleLogoutRetryInterval() + " milli seconds to " +
                ...
                if (!isSuccessfullyLogout) {
                    log.error("Single logout failed after retrying " + Util.getSingleLogoutRetryCount() +
                              " times with time interval " + Util.getSingleLogoutRetryInterval() + " in milli seconds.");
                }
