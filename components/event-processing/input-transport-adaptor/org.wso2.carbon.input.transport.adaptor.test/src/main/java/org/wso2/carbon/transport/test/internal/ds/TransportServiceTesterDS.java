...
import org.wso2.carbon.transport.test.TestInputTransportAdaptorAdaptorListener;
 ...
 * interface="org.wso2.carbon.transport.adaptor.core.InputTransportAdaptorService" cardinality="1..1"
...
public class TransportServiceTesterDS {
    ...
    protected void activate(ComponentContext context) {
        ...
        InputTransportAdaptorService inputTransportAdaptorService = TransportAdaptorHolder.getInstance().getInputTransportAdaptorService();

//        registerWso2EventReceiver(inputTransportAdaptorService);
        registerJMS(inputTransportAdaptorService);
    ...
    private void registerWso2EventReceiver(InputTransportAdaptorService inputTransportAdaptorService) {
...
//        inputTransportAdaptorConfiguration.addCommonAdaptorProperty("password", "admin1");
        ...
        try {
            inputTransportAdaptorService.subscribe(inputTransportAdaptorConfiguration, inputTransportMessageConfiguration, new TestInputTransportAdaptorAdaptorListener("wso2eventreceiver", "org.wso2.phone.retail.store:1.2.0"), TransportAdaptorHolder.getInstance().getConfigurationContextService().getServerConfigContext().getAxisConfiguration());
        } catch (InputTransportAdaptorEventProcessingException e) {
            log.error("Error occurred when subscribing " + e);
        }
    }

    private void registerJMS(InputTransportAdaptorService inputTransportAdaptorService) {
        InputTransportAdaptorConfiguration inputTransportAdaptorConfiguration = new InputTransportAdaptorConfiguration();
    ...
    protected void setTransportAdaptorService(InputTransportAdaptorService inputTransportAdaptorService) {
        TransportAdaptorHolder.getInstance().registerTransportService(inputTransportAdaptorService);
    }

    protected void unSetTransportAdaptorService(InputTransportAdaptorService inputTransportAdaptorService) {
        TransportAdaptorHolder.getInstance().unRegisterTransportService(inputTransportAdaptorService);
    }
