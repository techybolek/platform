...
public class CarbonInputTransportAdaptorService implements InputTransportAdaptorService {
    ...
    private static final String INPUT_TRANSPORT_ADAPTOR = "Input Transport Adaptor";
    ...
    public String subscribe(InputTransportAdaptorConfiguration inputTransportAdaptorConfiguration,
        ...
        if (inputTransportAdaptorConfiguration.isEnableStatistics()) {
            int tenantId = PrivilegedCarbonContext.getCurrentContext(axisConfiguration).getTenantId();
            EventStatisticsMonitor statisticsMonitor = InputTransportAdaptorServiceValueHolder.getEventStatisticsService().getEventStatisticMonitor(tenantId, INPUT_TRANSPORT_ADAPTOR, inputTransportAdaptorConfiguration.getName(), null);
            String tracerPrefix = tenantId + ":" + INPUT_TRANSPORT_ADAPTOR + ":" + inputTransportAdaptorConfiguration.getName() + ", received " + System.getProperty("line.separator");
            inputTransportAdaptorListener.setStatisticsMonitor(statisticsMonitor);
            inputTransportAdaptorListener.setTracerPrefix(tracerPrefix);
        }
