...
public final class InputTransportAdaptorConfigurationFilesystemInvoker {
    ...
    public static void save(String transportAdaptor, String transportName,
        ...
        try {
            ...
            out.close();
            log.info("Input Transport Adaptor configuration saved in th filesystem : "+ transportName);
            ...
            log.error("Could not save Input Transport Adaptor configuration " + transportName, e);
            throw new InputTransportAdaptorManagerConfigurationException("Error while saving : " +e.getMessage(), e);
        }
    }
    ...
    public static void deleteTransportAdaptorFile(String filePath,
        ...
        try {
            ...
            if (file.exists()) {
                ...
                if (!fileDeleted) {
                    log.error("Could not delete Input Transport Adaptor configuration : " + fileName);
                } else {
                    log.info("Input Transport Adaptor configuration deleted from file system : "+fileName);
                    InputTransportAdaptorDeployer deployer = (InputTransportAdaptorDeployer) getDeployer(axisConfiguration, InputTransportAdaptorManagerConstants.TM_ELE_DIRECTORY);
            ...
            throw new InputTransportAdaptorManagerConfigurationException("Error while deleting the Input Transport Adaptor " + e.getMessage(),e);
        }
    }
