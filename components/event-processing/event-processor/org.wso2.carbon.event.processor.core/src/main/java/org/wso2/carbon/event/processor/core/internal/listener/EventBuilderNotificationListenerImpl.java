...
public class EventBuilderNotificationListenerImpl implements EventBuilderNotificationListener {
    ...
    @Override
    public void configurationAdded(int tenantId, String streamId) {
        try {
            log.info("Trying to redeploy configuration files for stream: " + streamId);
            EventProcessorValueHolder.getEventProcessorService().addExternalStream(tenantId, streamId);
        } catch (Throwable e) {
    ...
    @Override
    public void configurationRemoved(int tenantId, String streamId) {
        try {
            log.info("Trying to undeploy execution plans for stream: " + streamId);
            EventProcessorValueHolder.getEventProcessorService().removeExternalStream(tenantId, streamId);
        } catch (Throwable e) {
            log.error(e.getMessage(), e);
        }
    }
