...
public class JDBCAuthorizationManager implements AuthorizationManager {
    ...
    public boolean isUserAuthorized(String userName, String resourceId, String action)
        ...
        String[] userRoles = userRealm.getUserStoreManager().getRoleListOfUser(unModifiedUser);
    ...
    private  void addAuthorizationForRole(String roleName, String resourceId, String action,
        ...
        try {
            ...
            if(isSystemRole){
                DatabaseUtil.updateDatabase(dbConnection, DBConstants.DELETE_ROLE_PERMISSION_WITH_DOMAIN_ID_SQL,
                                            UserCoreUtil.removeDomainFromName(roleName), resourceId, action,
                                            tenantId, tenantId, UserCoreConstants.SYSTEM_DOMAIN_ID);

                DatabaseUtil.updateDatabase(dbConnection, DBConstants.ADD_ROLE_PERMISSION_WITH_DOMAIN_SQL,
                                            permissionId, UserCoreUtil.removeDomainFromName(roleName),
                                            allow, tenantId,UserCoreConstants.SYSTEM_DOMAIN_ID);
            } else if (domain == null) {
                //if domain==null && !system role, it is from primary
                DatabaseUtil.updateDatabase(dbConnection, DBConstants.DELETE_ROLE_PERMISSION_WITH_DOMAIN_ID_SQL,
                                            UserCoreUtil.removeDomainFromName(roleName), resourceId, action,
                                            tenantId, tenantId, UserCoreConstants.PRIMARY_DOMAIN_ID);

                DatabaseUtil.updateDatabase(dbConnection, DBConstants.ADD_ROLE_PERMISSION_WITH_DOMAIN_SQL,
                                            permissionId, UserCoreUtil.removeDomainFromName(roleName),
                                            allow, tenantId,UserCoreConstants.PRIMARY_DOMAIN_ID);
            } else {
