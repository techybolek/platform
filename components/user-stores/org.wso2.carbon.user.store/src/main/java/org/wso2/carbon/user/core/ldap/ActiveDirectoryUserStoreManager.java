...
public class ActiveDirectoryUserStoreManager extends ReadWriteLDAPUserStoreManager {
	...
	public void doUpdateRoleName(String roleName, String newRoleName) throws UserStoreException {
		...
		try {
			...
			// rename the actual GroupNameAttribute
			Attributes modifyingAttributes = new BasicAttributes(true);
			Attribute roleNameAttribute = new BasicAttribute(roleNameAttributeName);
			roleNameAttribute.add(newRoleName);
			modifyingAttributes.put(roleNameAttribute);
			groupContext.modifyAttributes(groupNameRDN, DirContext.REPLACE_ATTRIBUTE,
					modifyingAttributes);
		...
		try {
			...
			for (Map.Entry<String, String> claimEntry : claims.entrySet()) {
				...
				if (claimMapping != null) {
					...
					// if mapped attribute is CN, then skip treating as a modified attribute -
    ...
    public Property[] getDefaultUserStoreProperties() {
        return ActiveDirectoryUserStoreConstants.ACTIVE_DIRECTORY_UM_PROPERTIES.toArray(new Property[ActiveDirectoryUserStoreConstants.ACTIVE_DIRECTORY_UM_PROPERTIES.size()]);
    }
