...
public class SecretManager {
	 ...
	 */
	public void init(Properties properties, Properties registryProperties,Resource resource) {
		...
		String configurationFile = MiscellaneousUtil.getProperty(properties,
				PROP_SECRET_MANAGER_CONF, PROP_DEFAULT_CONF_LOCATION);
		...
		for (String secretRepo : repositories) {
			...
			try {
				...
				RegistrySecretRepository instance = new RegistrySecretRepository(
						identityKeyStoreWrapper, trustKeyStoreWrapper);
				
				
				Enumeration eProps = registryProperties.propertyNames();
				while (eProps.hasMoreElements()) { 
				    String key = (String) eProps.nextElement(); 
				    String value = resource.getProperty(key);
					instance.getEncryptedData().put(key, value);
				}
				...
				handleException("A Secret Provider cannot be found for class name : "
						+ provider);
			}
		}
