...
package org.wso2.carbon.mediation.secure.vault.ui;
...
import org.wso2.carbon.mediation.secure.vault.SecretManagerInitializer;
import org.wso2.carbon.mediation.secure.vault.SecureVaultConstants;
import org.wso2.carbon.mediation.secure.vault.cipher.tool.CipherTool;
...
public class PropertiesServiceClient {
    ...
    public void setProperty (HttpServletRequest request) throws Exception {
        ...
        try {
            stub.setProperty(path, name, encrypted);
            //do refresh secret manager
            SecretManagerInitializer.getInstance().refresh();
        } catch (Exception e) {
    ...
    public void updateProperty(HttpServletRequest request) throws Exception {
        ...
        String encrypted = cipherTool.doEncryption(value);
        try {
            stub.updateProperty(path, name, encrypted, oldName);
            SecretManagerInitializer.getInstance().refresh();
        } catch (Exception e) {
