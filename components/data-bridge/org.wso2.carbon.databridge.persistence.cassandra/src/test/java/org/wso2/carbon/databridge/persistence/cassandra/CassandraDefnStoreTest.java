...
// save stream defn 1
    ...
    public void deleteStreamDefinitionAndId() throws StreamDefinitionStoreException {
        Collection<StreamDefinition> allStreamDefinitionFromStore = cassandraConnector.getAllStreamDefinitionFromStore(getCluster());
        int i = 0;
        List<String> streamKeys = new ArrayList<String>();
        for (StreamDefinition streamDefinition : allStreamDefinitionFromStore) {
            cassandraConnector.deleteStreamDefinitionFromStore(getCluster(), streamDefinition.getStreamId());
            i++;
            Collection<StreamDefinition> newAllStreamDefns = cassandraConnector.getAllStreamDefinitionFromStore(getCluster());
            assertEquals(allStreamDefinitionFromStore.size() - i, newAllStreamDefns.size());

            streamKeys.add(DataBridgeUtils.constructStreamKey(streamDefinition.getName(),
                                                              streamDefinition.getVersion()));
        }

        for (String streamKey : streamKeys) {
            cassandraConnector.deleteStreamIdFromStore(getCluster(), streamKey);
            assertNull(cassandraConnector.getStreamIdFromStore(getCluster(), streamKey));
        }
