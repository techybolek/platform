...
<%
    String configName = request.getParameter("eventBuilder");
    ...
    if (stub != null) {
        if (configName != null) {
            stub.removeEventBuilder(configName);
%>
<script type="text/javascript">CARBON.showInfoDialog('Event builder successfully deleted.');</script>
<%
        }

        EventBuilderConfigurationDto[] deployedEventBuilderConfigFiles = stub.getEventBuilderConfigurationsFor("DEPLOYED");
        if (deployedEventBuilderConfigFiles != null) {
            activeEventBuilders = deployedEventBuilderConfigFiles.length;
        }

        String[] notDeployedEventBuilderConfigFiles = stub.getUndeployedFileNames();
        if (notDeployedEventBuilderConfigFiles != null) {
            totalNotDeployedEventBuilders = notDeployedEventBuilderConfigFiles.length;
        }
    }
...
<div id="middle">
<div id="workArea">
    <h3><fmt:message key="available.event.builders.header"/></h3><br/>
    <h5 class="activeEventBuilders"><%=activeEventBuilders%> <fmt:message
            key="active.event.builders.header"/> , <% if (totalNotDeployedEventBuilders > 0) { %><a
            href="notdeployed_event_builder_files_details.jsp"><%=totalNotDeployedEventBuilders%>
        <fmt:message
    ...
    <table class="styledLeft">
        <%
            if (stub != null) {
                EventBuilderConfigurationDto[] eventBuilderConfigurationDtos = stub.getAllEventBuilderConfigurations();
        ...
        <tr>
            ...
            <th><fmt:message key="transport.adaptor.type.header"/></th>
        ...
        <tr>
            ...
            <td><%=eventBuilderConfigurationDto.getInputTransportAdaptorType()%>
            </td>
            <td>
        ...
        <thead>
        <tr>
            <th><fmt:message key="event.builder.noeb.header"/></th>
        </tr>
        </thead>
        <tbody>
        <tr>
