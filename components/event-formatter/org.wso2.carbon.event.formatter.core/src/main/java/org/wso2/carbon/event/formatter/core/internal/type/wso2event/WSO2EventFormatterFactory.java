...
public class WSO2EventFormatterFactory implements EventFormatterFactory {


    @Override
    public OutputMapping constructOutputMapping(OMElement omElement) {
        return WSO2EventFormatterConfigurationBuilder.fromOM(omElement);
    }

    @Override
    public OMElement constructOutputMappingOM(
            OutputMapping outputMapping, OMFactory factory) {
        return WSO2EventFormatterConfigurationBuilder.outputMappingToOM(outputMapping, factory);
    }

    @Override
    public EventFormatter constructEventFormatter(
            EventFormatterConfiguration eventFormatterConfiguration, int tenantId) {
        return new WSO2EventOutputEventFormatter(eventFormatterConfiguration, tenantId);
    }
}
