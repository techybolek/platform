...
public class TextEventFormatterFactory implements EventFormatterFactory {


    @Override
    public OutputMapping constructOutputMapping(OMElement omElement) {
        return TextFormatterConfigurationBuilder.fromOM(omElement);
    }

    @Override
    public OMElement constructOutputMappingOM(
            OutputMapping outputMapping, OMFactory factory) {
        return TextFormatterConfigurationBuilder.outputMappingToOM(outputMapping, factory);
    }

    @Override
    public EventFormatter constructEventFormatter(
            EventFormatterConfiguration eventFormatterConfiguration, int tenantId) {
        return new TextOutputEventFormatter(eventFormatterConfiguration, tenantId);
    }
}
