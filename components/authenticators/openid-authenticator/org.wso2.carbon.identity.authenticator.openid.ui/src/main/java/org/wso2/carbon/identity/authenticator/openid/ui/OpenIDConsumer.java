...
import org.wso2.carbon.identity.authenticator.openid.ui.dataobjects.OpenIDDO;
import org.wso2.carbon.identity.authenticator.openid.ui.extensions.OpenIDExtension;
...
public class OpenIDConsumer {
     ...
     */
    public OpenIDConsumer() throws OpenIDAuthenticationException {
        try {
            ...
            throw new OpenIDAuthenticationException("Relying Party initialization failed", e);
        }
     ...
     * @throws RelyingPartyException
     */
    public static OpenIDConsumer getInstance() throws OpenIDAuthenticationException {
    ...
    public String doOpenIDAuthentication(OpenIDAuthenticationRequest request)
        ...
        if (request == null)
            throw new OpenIDAuthenticationException("Invalid OpenID authentication request");
    ...
    public Message verifyOpenID(HttpServletRequest request, ParameterList openidResp, String receivingURL)
            throws OpenIDException, OpenIDAuthenticationException {
        ...
        discovered = (DiscoveryInformation) session.getAttribute(OpenIDConstants.DISC);
        ...
        if (authResponse.getAuthResultType() == AuthResponseHelper.ResultType.AUTH_SUCCESS) {
            ...
            throw new OpenIDAuthenticationException("OpenID verification failed");
        }
     ...
     * @throws RelyingPartyException
     */
    protected String authRequest(OpenIDAuthenticationRequest request) throws OpenIDAuthenticationException {
        ...
        try {
            ...
            session.setAttribute(OpenIDConstants.DISC, helper.getDiscoveryInformation());
            ...
            // Present error to the user
            throw new OpenIDAuthenticationException(e.getMessage(), e);
        }
    }
     ...
     */
    public OpenIDDO validateOpenIDAuthentication(HttpServletRequest request, String requestURL)
            throws OpenIDAuthenticationException {
        ...
        List<OpenIDClaimDO> claimList = null;
        OpenIDDO openID = null;
        ...
        try {
            ...
            mode = response.getParameterValue(OpenIDConstants.ATTR_MODE);

            if (mode != null && OpenIDConstants.CANCEL.equals(mode)) {
                // User has denied sending his profile info :(
                throw new OpenIDAuthenticationException("User has denied sending his profile info");
            }

            authSuccess = (AuthSuccess) verifyOpenID(request, response, requestURL);

            claimList = new ArrayList<OpenIDClaimDO>();
            ...
            openID.setClaims(claimList.toArray(new OpenIDClaimDO[claimList.size()]));
            ...
            // Present error to the user.
            throw new OpenIDAuthenticationException("OpenID authentication failed", e);
        }
    }
