...
import org.wso2.carbon.identity.authenticator.openid.ui.extensions.OpenIDAttributeExchange;
import org.wso2.carbon.identity.authenticator.openid.ui.extensions.OpenIDExtension;
import org.wso2.carbon.identity.authenticator.openid.ui.extensions.OpenIDPape;
import org.wso2.carbon.identity.authenticator.openid.ui.extensions.OpenIDSimpleReg;
...
public class OpenIDExtensionFactory {
	...
	public OpenIDExtension getExtension(String type) {
		...
		if (OpenIDConstants.ATTRIBUTE_EXCHANGE.equals(type)) {
			return new OpenIDAttributeExchange();
		} else if (OpenIDConstants.SIMPLE_REGISTRATION.equals(type)) {
			return new OpenIDSimpleReg();
		} else if (OpenIDConstants.PAPE.equals(type)) {
	...
	public OpenIDExtension getExtension(String alias, AuthSuccess auth) {
		...
		if (alias.equals(AxMessage.OPENID_NS_AX)
				|| alias.equals(OpenIDConstants.ExchangeAttributes.NS_AX)) {
			return new OpenIDAttributeExchange(auth);
		} else if (alias.equals(OpenIDConstants.SimpleRegAttributes.NS_SREG)
				|| alias.equals(SRegMessage.OPENID_NS_SREG)
				|| alias.equals(OpenIDConstants.SimpleRegAttributes.NS_SREG_1)) {
