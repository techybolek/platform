...
import org.wso2.carbon.identity.authenticator.openid.ui.OpenIDConstants;
...
public class OpenIDSimpleReg implements OpenIDExtension {
    ...
    public void setSessionAttributes(List<OpenIDClaimDO> claimList) throws OpenIDAuthenticationException {
        ...
        try {
            ...
            } else if (authSuccess.hasExtension(OpenIDConstants.SimpleRegAttributes.NS_SREG)) {
                sregResp = (SRegResponse) authSuccess
                        .getExtension(OpenIDConstants.SimpleRegAttributes.NS_SREG);

            } else if (authSuccess.hasExtension(OpenIDConstants.SimpleRegAttributes.NS_SREG_1)) {
                sregResp = (SRegResponse) authSuccess
                        .getExtension(OpenIDConstants.SimpleRegAttributes.NS_SREG_1);
            }

            if (sregResp != null) {
                OpenIDClaimDO claim = null;
                attributes = sregResp.getAttributes();
                iterator = attributes.entrySet().iterator();
                while (iterator.hasNext()) {
                    entry = (Entry) iterator.next();
                    claim = new OpenIDClaimDO();
