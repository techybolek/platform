...
public class WSExecutor implements Execution {
	...
	public boolean execute(RequestContext context, String currentState,
		...
		if (!isAsynchronous) {
			...
			try {
				if (responseXpath != null && responseNamespacePrefix != null
						&& responseNamespace != null) {
					xpathExpression = new AXIOMXPath(responseXpath);
					xpathExpression.addNamespace(responseNamespacePrefix,
							responseNamespace);
				} else {
					xpathExpression = null;
				}
				synchronousExecuteWS(getEPR(context),
						getParameterizedPayload(context), context,
						xpathExpression);
	 ...
	 * the registry
	 * 
	 * @param endPointReferance
	 *            - the end point reference of the web service
	 * @param payload
	 *            - the payload needed to invoke the web service
	 * @param context
	 *            - the request context
	 * @param responseDestinationPath
	 *            - the registry path to which the response value is to be saved
	 * @param saveResponseAs
	 *            - define whether the response is to be saved as an attribute
	 *            or property
	 * @param responseName
	 *            - the name of the attribute or property
	 * @param xpathExpression
	 *            - the xpath expression to extract the value from the response
	 */
	private void synchronousExecuteWS(final String endPointReferance,
		...
		try {
			...
			if (arrayList.isEmpty()) {
				isWSSuccessful = false;
				log.error("Service invocation was not succesful");
				return;
			}
