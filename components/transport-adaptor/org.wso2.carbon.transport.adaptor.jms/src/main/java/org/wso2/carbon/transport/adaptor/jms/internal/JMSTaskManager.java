...
import org.wso2.carbon.transport.adaptor.jms.internal.util.JMSConstants;
import org.wso2.carbon.transport.adaptor.jms.internal.util.JMSUtils;
...
public class JMSTaskManager {
    ...
    public synchronized void start() {
        ...
        switch (cacheLevel) {
            case JMSConstants.CACHE_NONE:
                log.debug("No JMS resources will be cached/shared between poller " +
                          "worker tasks of transport adaptor : " + transportAdaptorName);
                break;
            ...
            default: {
                handleException("Invalid cache level : " + cacheLevel +
                                " for transport adaptor : " + transportAdaptorName);
            }
        }
        ...
        log.info("Task manager for transport adaptor : " + transportAdaptorName + " [re-]initialized");
    }
    ...
    public synchronized void stop() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Stopping JMSTaskManager for transport adaptor : " + transportAdaptorName);
        }
        ...
        if (activeTaskCount > 0) {
            log.warn("Unable to shutdown all polling tasks of transport adaptor : " + transportAdaptorName);
        }

        if (JMSTaskManagerState != STATE_FAILURE) {
            JMSTaskManagerState = STATE_STOPPED;
        }
        log.info("Task manager for transport adaptor : " + receiveTimeout + " shutdown");
    }
    ...
    private class MessageListenerTask implements Runnable, ExceptionListener {
            ...
            if (isActive()) {
                if (connection != null && cacheLevel < JMSConstants.CACHE_CONNECTION) {
                    try {
                        ...
                        log.warn("Error pausing Message Listener task for transport adaptor : " + transportAdaptorName);
                    }
                }
        ...
        public void resume() {
            if (connection != null && cacheLevel < JMSConstants.CACHE_CONNECTION) {
                try {
                    ...
                    log.warn("Error resuming Message Listener task for transport adaptor : " + transportAdaptorName);
                }
            }
        ...
        private Message receiveMessage() {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Waiting for a message for transport adaptor : " + transportAdaptorName + " - duration : "
            ...
            try {
                ...
                logError("Error receiving message for transport adaptor : " + transportAdaptorName, e);
            }
            return null;
        }
        ...
        public void onException(JMSException j) {
            ...
            do {
                try {
                    log.info("Reconnection attempt : " + r + " for transport adaptor : " + transportAdaptorName);
                    start();
                ...
                if (!connected) {
                    ...
                    log.error("Reconnection attempt : " + (r++) + " for transport adaptor : " + transportAdaptorName +
        ...
        private void closeConnection() {
            if (connection != null &&
                ...
                try {
                    if (log.isDebugEnabled()) {
                        log.debug("Closing non-shared JMS connection for transport adaptor : " + transportAdaptorName);
                    }
                    connection.close();
                } catch (JMSException e) {
        ...
        private void closeSession(boolean forced) {
            if (session != null &&
                ...
                try {
                    if (log.isDebugEnabled()) {
                        log.debug("Closing non-shared JMS session for transport adaptor : " + transportAdaptorName);
                    }
                    session.close();
                } catch (JMSException e) {
        ...
        private void closeConsumer(boolean forced) {
            if (consumer != null &&
                ...
                try {
                    if (log.isDebugEnabled()) {
                        log.debug("Closing non-shared JMS consumer for transport adaptor : " + transportAdaptorName);
                    }
        ...
        private Connection createConnection() {
            ...
            try {
                ...
                connection.start();
                log.debug("JMS Connection for transport adaptor : " + transportAdaptorName + " created and started");
        ...
        private Session createSession() {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Creating a new JMS Session for transport adaptor : " + transportAdaptorName);
                }
                return JMSUtils.createSession(
                        connection, isSessionTransacted(), getSessionAckMode(), isJmsSpec11(), isQueue());

            } catch (JMSException e) {
                handleException("Error creating JMS session for transport adaptor : " + transportAdaptorName, e);
            }
            return null;
        }
        ...
        private MessageConsumer createConsumer() {
            try {
                if (log.isDebugEnabled()) {
                    log.debug("Creating a new JMS MessageConsumer for transport adaptor : " + transportAdaptorName);
                }
                ...
                MessageConsumer consumer = JMSUtils.createConsumer(
                        ...
                        (isSubscriptionDurable() && getDurableSubscriberName() != null ?
                         getDurableSubscriberName() : transportAdaptorName),
                        getMessageSelector(), isPubSubNoLocal(), isSubscriptionDurable(), isJmsSpec11());
                consumerCount.incrementAndGet();
                return consumer;

            } catch (JMSException e) {
                handleException("Error creating JMS consumer for transport adaptor : " + transportAdaptorName, e);
            }
            return null;
        }
    }
    ...
    /**
     * Get the InitialContext for lookup using the JNDI parameters applicable to the transport adaptor
     *
            ...
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("JMS Destination with JNDI name : " + getDestinationJNDIName() +
                              " found for transport adaptor " + transportAdaptorName);
                }
            } catch (NamingException e) {
                try {
    ...
    private UserTransaction getUserTransaction() {
        if (!cacheUserTransaction) {
            if (log.isDebugEnabled()) {
                log.debug("Acquiring a new UserTransaction for transport adaptor : " + transportAdaptorName);
            }
        ...
        if (sharedUserTransaction == null) {
            try {
                ...
                sharedUserTransaction =
                        JMSUtils.lookup(context, UserTransaction.class, getUserTransactionJNDIName());
                if (log.isDebugEnabled()) {
                    log.debug("Acquired shared UserTransaction for transport adator : " + transportAdaptorName);
                }
            } catch (NamingException e) {
                handleException("Error looking up UserTransaction : " + getUserTransactionJNDIName() +
                                " using JNDI properties : " + jmsProperties, e);
    ...
    private void handleException(String msg, Exception e) {
        log.error(msg, e);
        throw new TransportAdaptorEventProcessingException(msg, e);
    }

    private void handleException(String msg) {
        log.error(msg);
        throw new TransportAdaptorEventProcessingException(msg);
    }
