...
//    public void appendServiceConfig(Node servicesConfigNode) {
//
//        // Building custom services configuration
//        for (Node serviceNode : servicesConfigNode.getChildNodes()) {
//            // skip default node
//            if (!defaultServiceConfig.getServiceName().equals(serviceNode.getName())) {
//
//                String serviceName = serviceNode.getName();
//
//                // reading domains
...
//                        // this is probably a mistake, so we don't proceed
//                        String msg =
//                            "Duplicated host names detected for different service domains.\n" +
//                                "Element: \n" + serviceNode.toString();
//                        log.error(msg);
//                        throw new RuntimeException(msg);
//                    }
...
//    }
    ...
    public boolean addServiceConfiguration(ServiceConfiguration serviceConfig) {
        ...
        if (map.containsKey(subDomain)) {
            if (!map.get(subDomain).equals(serviceConfig)) {
                log.debug("Service configuration will be replaced, with the latest.\n" +
                    "Old Service Config: " + map.get(subDomain).toString() + "\n" +
                    "New Service Config: " + serviceConfig.toString());
...
//            }
        ...
        public String toString() {
            ...
            while (hosts.iterator().hasNext()) {
                hostStr.append(hosts.iterator().next() + ", ");
            }

            return toString("domain", domain) +
                toString("subDomain", subDomain) +
                toString("serviceName", serviceName) +
                toString("alarmingLowerRate", alarmingLowerRate) +
                toString("alarmingUpperRate", alarmingUpperRate) +
                toString("availability_zone", availability_zone) +
                toString("roundsToAverage", roundsToAverage) +
                toString("maxAppInstances", maxAppInstances) +
                toString("minAppInstances", minAppInstances) +
                toString("scaleDownFactor", scaleDownFactor) +
                toString("publicIp", publicIp) +
                toString("maxRequestsPerSecond", maxRequestsPerSecond) +
                toString("tenantRange", tenantRange) +
                toString("instancesPerScaleUp", instancesPerScaleUp) +
                toString("messageExpiryTime", messageExpiryTime) +
                toString("hosts", hostStr.toString());
        }
        ...
        public boolean equals(ServiceConfiguration config) {
            return this.domain.equals(config.getDomain()) &&
                   ...
                   this.serviceName.equals(config.getServiceName()) &&
                   this.alarmingLowerRate == config.getAlarmingLowerRate() &&
                   this.alarmingUpperRate == config.getAlarmingUpperRate() &&
                   this.availability_zone.equals(config.getAvailability_zone()) &&
                   this.roundsToAverage == config.getRoundsToAverage() &&
                   this.maxAppInstances == config.getMaxAppInstances() &&
                   this.minAppInstances == config.getMinAppInstances() &&
                   this.scaleDownFactor == config.getScaleDownFactor() &&
                   this.publicIp.equals(config.getPublicIp()) &&
                   this.maxRequestsPerSecond == config.getMaxRequestsPerSecond() &&
                   this.tenantRange.equals(config.getTenantRange()) &&
                   this.instancesPerScaleUp == config.getInstancesPerScaleUp() &&
                   this.messageExpiryTime == config.getMessageExpiryTime() &&
                   this.hosts.containsAll(config.getHosts())
                   ;
        }
        ...
        private String toString(String name, Object val){
            return name + " : "+val+"\n";
        }
    }
