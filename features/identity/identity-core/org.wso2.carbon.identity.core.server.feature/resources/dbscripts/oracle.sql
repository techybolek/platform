...
CREATE TABLE IF NOT EXISTS UM_TENANT_IDP (
			...
			PRIMARY KEY (UM_ID),
			CONSTRAINT CON_APP_KEY UNIQUE (UM_TENANT_ID, UM_TENANT_IDP_ISSUER));
...
CREATE TABLE IF NOT EXISTS UM_TENANT_IDP_ROLES_MAPPING (
			UM_ID INTEGER NOT NULL,
...
CREATE SEQUENCE UM_TENANT_IDP_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE SEQUENCE TENANT_IDP_ROLES_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE SEQUENCE UM_TENANT_IDP_ROLES_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE;

CREATE OR REPLACE TRIGGER UM_TENANT_IDP_TRIGGER
                    BEFORE INSERT
                    ON UM_TENANT_IDP
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                     BEGIN
                       SELECT UM_TENANT_IDP_SEQUENCE.nextval INTO :NEW.UM_ID FROM dual;
 			   END;

CREATE OR REPLACE TRIGGER UM_TENANT_IDP_ROLES_TRIGGER
                    BEFORE INSERT
                    ON UM_TENANT_IDP_ROLES
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                     BEGIN
                       SELECT UM_TENANT_IDP_ROLES_SEQUENCE.nextval INTO :NEW.UM_ID FROM dual;
 			   END;

CREATE OR REPLACE TRIGGER UM_TENANT_IDP_ROLES_MAPPING_TRIGGER
                    BEFORE INSERT
                    ON UM_TENANT_IDP_ROLES_MAPPING
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                     BEGIN
                       SELECT UM_TENANT_IDP_ROLES_MAPPING_SEQUENCE.nextval INTO :NEW.UM_ID FROM dual;
 			   END;
