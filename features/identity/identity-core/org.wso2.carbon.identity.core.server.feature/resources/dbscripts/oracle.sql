...
CREATE TABLE UM_TENANT_IDP (
			...
			UM_TENANT_IDP_PRIMARY TINYINT NOT NULL,
			PRIMARY KEY (UM_ID),
			CONSTRAINT CON_IDP_KEY UNIQUE (UM_TENANT_ID, UM_TENANT_IDP_ISSUER))
/
...
/
CREATE TABLE UM_TENANT_IDP_ROLES_MAPPING (
			UM_ID INTEGER NOT NULL,
			UM_TENANT_IDP_ROLE_ID INTEGER NOT NULL,
			UM_TENANT_ID INTEGER NOT NULL,
			UM_TENANT_ROLE VARCHAR(512),	
			PRIMARY KEY (UM_ID),
			CONSTRAINT CON_ROLES_MAPPING_KEY UNIQUE (UM_TENANT_IDP_ROLE_ID, UM_TENANT_ID, UM_TENANT_ROLE),
...
/
CREATE SEQUENCE IDP_ROLES_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1 NOCACHE
/
...
CREATE OR REPLACE TRIGGER IDP_ROLES_TRIGGER
 			   ...
 			   END
/
CREATE OR REPLACE TRIGGER IDP_ROLES_MAPPING_TRIGGER
                    BEFORE INSERT
                    ON UM_TENANT_IDP_ROLES_MAPPING
                    REFERENCING NEW AS NEW
                    FOR EACH ROW
                     BEGIN
                       SELECT IDP_ROLES_MAPPING_SEQUENCE.nextval INTO :NEW.UM_ID FROM dual;
 			   END
/
