...
public class BpsSetter extends EnvironmentSetter {
    ...
    public BpsSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("bps.service.host.name", "process.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("bps.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("bps.http.port", "9763"));
        String httpsPorts = (prop.getProperty("bps.https.port", "9400"));
        String webContextRoots = (prop.getProperty("bps.webContext.root", null));
