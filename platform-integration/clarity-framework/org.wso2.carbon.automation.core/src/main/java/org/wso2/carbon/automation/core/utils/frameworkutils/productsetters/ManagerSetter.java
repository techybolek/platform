...
public class ManagerSetter extends EnvironmentSetter {
    ...
    public ManagerSetter() {
        this.properties = new ProductUrlGeneratorUtil().getStream();
        String hostNames;
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("manager.service.host.name", "stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("manager.host.name", "localhost"));
        }

        String httpPorts = (prop.getProperty("manager.http.port", "9763"));
        String httpsPorts = (prop.getProperty("manager.https.port", "9443"));
        String webContextRoots = (prop.getProperty("manager.webContext.root", null));
