...
public class GsSetter extends EnvironmentSetter {
    ...
    public GsSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("gs.service.host.name", "gadget.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("gs.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("gs.http.port", "9763"));
        String httpsPorts = (prop.getProperty("gs.https.port", "9443"));
        String webContextRoots = (prop.getProperty("gs.webContext.root", null));
