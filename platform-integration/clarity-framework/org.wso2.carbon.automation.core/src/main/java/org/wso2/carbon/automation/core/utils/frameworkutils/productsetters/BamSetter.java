...
public class BamSetter extends EnvironmentSetter {
    ...
    public BamSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("bam.service.host.name", "monitor.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("bam.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("bam.http.port", "9770"));
        String httpsPorts = (prop.getProperty("bam.https.port", "9450"));
        String webContextRoots = (prop.getProperty("bam.webContext.root", null));
