...
public class MsSetter extends EnvironmentSetter {
    ...
    public MsSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("ms.service.host.name", "mashup.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("ms.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("ms.http.port", "9763"));
        String httpsPorts = (prop.getProperty("ms.https.port", "9443"));
        String webContextRoots = (prop.getProperty("ms.webContext.root", null));
