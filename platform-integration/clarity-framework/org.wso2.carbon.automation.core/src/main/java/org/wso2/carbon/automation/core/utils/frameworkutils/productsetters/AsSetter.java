...
public class AsSetter extends EnvironmentSetter {
    ...
    public AsSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("app.service.host.name", "appserver.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("as.host.name", "localhost"));
        }

        String httpPorts = (properties.getProperty("as.http.port", "9764"));
        String httpsPorts = (properties.getProperty("as.https.port", "9443"));
        String webContextRoots = (prop.getProperty("as.webContext.root", null));
