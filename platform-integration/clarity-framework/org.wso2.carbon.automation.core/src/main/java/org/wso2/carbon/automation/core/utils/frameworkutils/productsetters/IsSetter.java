...
public class IsSetter extends EnvironmentSetter {
    ...
    public IsSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("is.service.host.name", "identity.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("is.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("is.http.port", "9763"));
        String httpsPorts = (prop.getProperty("is.https.port", "9443"));
        String webContextRoots = (prop.getProperty("is.webContext.root", null));
