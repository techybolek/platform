...
package org.wso2.carbon.tests.dss;
...
public class TestExecutorDss extends MasterTestSuite {
    private static final Log log = LogFactory.getLog(TestExecutorDss.class);

    @AfterSuite
    public void execute() {
        System.setProperty("server.list", "DSS");
        System.setProperty("framework.resource.location", resourcePath);
        System.setProperty("clarity.property.file", clarityPropertypath);
        ...
        for (File jarFile : jarLoader.getJarList()) {
            ...
            suiteList.addAll(testSuiteFinder.initializeSuitesAndJarFile(jarResourcepath));
            TestNG testNG = superSuite("TestNG-DSS-Suite", suiteList);
            testNG.run();
        }
    }
