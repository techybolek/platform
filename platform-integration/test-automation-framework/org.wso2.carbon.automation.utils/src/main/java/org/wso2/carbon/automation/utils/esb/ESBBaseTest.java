...
public class ESBBaseTest {
    ...
    public EnvironmentVariables esbServer;
    ...
    public ESBBaseTest(int userId) throws Exception {
        axis2Client = new StockQuoteClient();
        userInfo = UserListCsvReader.getUserInfo(userId);
        EnvironmentBuilder builder = new EnvironmentBuilder().esb(userId);
        esbServer = builder.build().getEsb();
    ...
    public String getMainSequenceURL() {
    ...
    public String getProxyServiceURL(String proxyServiceName) {
        return esbServer.getServiceUrl() + "/" + proxyServiceName;
    }

    public String getProxyServiceSecuredURL(String proxyServiceName) {
        return esbServer.getSecureServiceUrl() + "/" + proxyServiceName;
    }

    public void loadSampleESBConfiguration(int sampleNo) throws Exception {
        OMElement synapseSample = esbUtils.loadSampleESBConfiguration(sampleNo);
        updateESBConfiguration(synapseSample);

    }

    public void loadESBConfigurationFromClasspath(String relativeFilePath) throws Exception {
    ...
    public void addProxyService(OMElement proxyConfig) throws Exception {
    ...
    public void isProxyDeployed(String proxyServiceName) throws Exception {
    ...
    public void deleteProxyService(String proxyServiceName) throws Exception {
    ...
    public void addSequence(OMElement sequenceConfig) throws Exception {
    ...
    public void addEndpoint(OMElement endpointConfig)
            throws Exception {
    ...
    public void addLocalEntry(OMElement localEntryConfig) throws Exception {
    ...
    public void addMessageProcessor(OMElement messageProcessorConfig) throws Exception {
    ...
    public void addMessageStore(OMElement messageStoreConfig) throws Exception {
    ...
    public void addSequenceTemplate(OMElement sequenceTemplate) throws Exception {
    ...
    public void addApi(OMElement api) throws Exception {
    ...
    public void addPriorityExecutor(OMElement priorityExecutor) throws Exception {
    ...
    public void applySecurity(String serviceName, int policyId, String[] userGroups)
            throws SecurityAdminServiceSecurityConfigExceptionException, RemoteException,
                   InterruptedException {
        SecurityAdminServiceClient securityAdminServiceClient = new SecurityAdminServiceClient(esbServer.getBackEndUrl(), esbServer.getSessionCookie());
    ...
    private void deletePriorityExecutors() {
        if (priorityExecutorList != null) {
            for (String executor : priorityExecutorList) {
                try {
    ...
    public String getESBResourceLocation() {
        return ProductConstant.getResourceLocations(ProductConstant.ESB_SERVER_NAME);
    }

    public String getBackEndServiceUrl(String serviceName) {
        return EndpointGenerator.getEndpointServiceUrl(serviceName);
    }

    public OMElement setEndpoints(OMElement synapseConfig) throws XMLStreamException {
        if (isBuilderEnabled()) {
    ...
    public DataHandler setEndpoints(DataHandler dataHandler)
    ...
    private String replaceEndpoints(String config) {
        String stockQuoteService = getBackEndServiceUrl(ESBTestConstant.SIMPLE_STOCK_QUOTE_SERVICE);
        String secureStockQuoteService = getBackEndServiceUrl(ESBTestConstant.SECURE_STOCK_QUOTE_SERVICE);

        config = config.replace("http://localhost:9000/services/SimpleStockQuoteService"
                , stockQuoteService);
        config = config.replace("http://127.0.0.1:9000/services/SimpleStockQuoteService"
                , stockQuoteService);
        config = config.replace("http://localhost:9000/services/SecureStockQuoteService"
                , secureStockQuoteService);
        config = config.replace("http://127.0.0.1:9000/services/SecureStockQuoteService"
                , secureStockQuoteService);
        return config;
    }
