...
public class StoreSetter extends EnvironmentSetter {
    ...
    public StoreSetter() {
        ...
        if (Boolean.parseBoolean(prop.getProperty("stratos.test"))) {
            hostNames = (properties.getProperty("store.service.host.name", "storage.stratoslive.wso2.com"));
        } else {
            hostNames = (properties.getProperty("store.host.name", "localhost"));
        }
        String httpPorts = (prop.getProperty("store.http.port", "9763"));
        String httpsPorts = (prop.getProperty("store.https.port", "9443"));
        String webContextRoots = (prop.getProperty("store.webContext.root", null));
