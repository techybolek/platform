...
public class WebResourceSampleTestCase extends DSSIntegrationTest {
    private static String serviceName = "WebResourceSample";
    ...
    public void selectOperation() throws AxisFault, InterruptedException {
        OMFactory fac = OMAbstractFactory.getOMFactory();
        OMNamespace omNs = fac.createOMNamespace("http://www.w3.org/2005/08/addressing", "ns1");
        OMElement payload = fac.createOMElement("getWheatherInfo", omNs);
        Thread.sleep(1000);
        OMElement result = new AxisServiceClient().sendReceive(payload,dssServer.getServiceUrl()+"/"+serviceName, "getWheatherInfo");
        Assert.assertTrue(result.toString().contains("<Region>New York</Region>"));
        Assert.assertTrue(result.toString().contains("<Region>Los Angeles</Region>"));
