...
function setup_as {
unset OPTIND
while getopts w:r:e:v:h: option
do
        case "${option}"
        in
...
#configure dev cloud
mkdir $working_dir/${environment}-cloud
...
cat $resorce_dir/configs/${environment}-cloud-carbon.xml | sed -e "s@AF_HOST@$af_host_name@g" > $carbon_home/repository/conf/carbon.xml
