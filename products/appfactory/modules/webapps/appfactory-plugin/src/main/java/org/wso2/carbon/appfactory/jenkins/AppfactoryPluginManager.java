    ...
    public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
        ...
        if (build.getResult() == Result.SUCCESS) {
            ...
            if (doDeploy) {
                ...
                String deployAction = build.getEnvironment(listener).get("deployAction");
    ...
    public void sendMessageToDeploy(final String applicationId,
        ...
        Thread clientCaller=new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    //This is required whenever auto build  triggers auto deploy.Deploy message should be send after finishing
                    //job execution
                    Thread.sleep(1000);
                    String applicationDeployerEPR = getDescriptor().getAppfactoryServerURL() +
                            "/services/ApplicationDeployer";
                    ...
                    CarbonUtils.setBasicAccessSecurityHeaders(getDescriptor().getAdminUserName(),
                            getDescriptor().getAdminPassword(),
                            deployerClient);
                    clientStub.deployArtifact(applicationId, stage, version, tagName, deployAction);

                } catch (AxisFault axisFault) {
                    log.error("Error in notifying deploy for application "+applicationId+" version"+version,axisFault);
                } catch (RemoteException e) {
                    log.error("Remote error in notifying deploy for application "+applicationId+" version"+version,e);
                } catch (ApplicationDeployerAppFactoryExceptionException e) {
                    log.error("Error in notifying deploy for application "+applicationId+" version"+version,e);
                } catch (InterruptedException e) {
                    log.error("Thread is interrupted in notifying deploy for application "+applicationId+" version"+version,e);
                }
            }
        }) ;
        clientCaller.start();
