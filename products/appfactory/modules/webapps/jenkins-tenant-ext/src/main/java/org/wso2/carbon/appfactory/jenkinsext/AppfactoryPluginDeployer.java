...
public class AppfactoryPluginDeployer {
	 ...
	 * @param dest
	 * @throws IOException
	 *             - if the error occurred while copying plugins to {@code dest}
	 *             folder.
	 *             IllegalArgumentException - invalid plugin source location is
	 *             given.
	 */
	protected void copyBundledPlugin(String src, String dest) throws IOException {
		...
		if (!source.exists()) {
			throw new IllegalArgumentException("Common plugin location cannot be found at " +
			                                   pluginPath);
		}
