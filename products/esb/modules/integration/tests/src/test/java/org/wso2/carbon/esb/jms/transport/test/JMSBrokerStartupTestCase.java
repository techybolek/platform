    ...
    public void startJMSBroker() throws Exception {
        ...
        if (FrameworkFactory.getFrameworkProperties(ProductConstant.ESB_SERVER_NAME).getEnvironmentSettings()
            ...
            serverManager.applyConfiguration(new File(ProductConstant.getResourceLocations(ProductConstant.ESB_SERVER_NAME)
                                                      + File.separator + "jms" + File.separator + "transport" + File.separator + "axis2.xml"));

        } else {
            //configure MB server to work with esb
            builder = new EnvironmentBuilder().mb(ProductConstant.ADMIN_USER_ID);
