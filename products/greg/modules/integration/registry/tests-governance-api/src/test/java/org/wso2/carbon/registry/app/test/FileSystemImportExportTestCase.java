...
public class FileSystemImportExportTestCase {
    ...
    private static String EXPORT_PATH = ProductConstant.SYSTEM_TEST_RESOURCE_LOCATION;
    ...
    public void FileImportTest() throws RegistryException, MalformedURLException {
        ...
        String filePath = this.getClass().getResource
                (File.separator + "artifacts" + File.separator + "GREG" + File.separator + "apptestresources").getPath();
    ...
    public void FileExportTest() throws RegistryException, FileNotFoundException {
        ...
        String fileExportPath = EXPORT_PATH + "/export";

        File file = new File(EXPORT_PATH + "/export");
        String osName = "";
        try {
        ...
        File f = new File(EXPORT_PATH + "/export/dbscripts/h2.sql");
        assertTrue(f.exists(), "File doesn't exist at the location");

        File f1 = new File(EXPORT_PATH + "/export/dbscripts/mysql.sql");
        assertTrue(f1.exists(), "File doesn't exist at the location");

        assertTrue(fileContainString(EXPORT_PATH + "/export/dbscripts/oracle.sql", "CREATE"), "Resource contain not found");
        assertTrue(fileContainString(EXPORT_PATH + "/export/dbscripts/h2.sql", "CREATE"), "Resource contain not found");
    ...
    public void JarFileExportTest() throws RegistryException, FileNotFoundException {
        ...
        String filePath = EXPORT_PATH + "/repository/lib/" + jarFileName;
        File file = new File(filePath);
        RegistryClientUtils.importToRegistry(file, "/framework", registry);
        ...
        File file2 = new File(EXPORT_PATH + "/export");
        RegistryClientUtils.exportFromRegistry(file2, "/framework", registry);

        File f1 = new File(EXPORT_PATH + "/export/" + jarFileName);
        assertTrue(f1.exists(), "File doesn't exist at the location");
