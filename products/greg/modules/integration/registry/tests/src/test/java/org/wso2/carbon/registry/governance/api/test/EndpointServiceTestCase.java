...
public class EndpointServiceTestCase {
    ...
    public void testAddDuplicateEndpoints() throws GovernanceException {
        ...
        assertTrue(endpointManagerDuplicate.getEndpoint(ep2.getId()).getQName().toString().
                contains("http://wso2.automation.endpoint"), "Endpoint not found");

        //delete endpoint
        endpointManagerDuplicate.removeEndpoint(ep2.getId());
        assertNull(endpointManagerDuplicate.getEndpoint(ep2.getId()), "Endpoint not removed");
    }
    ...
    private void removeServices() throws GovernanceException {
        ServiceManager serviceManager = new ServiceManager(governance);
