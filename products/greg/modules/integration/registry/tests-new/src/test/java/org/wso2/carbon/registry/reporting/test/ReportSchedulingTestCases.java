...
public class ReportSchedulingTestCases extends ReportingTestCaseSuper {
    ...
    public void testScheduleReport() throws Exception {
        ...
        if (framework.getEnvironmentSettings().is_runningOnStratos()) {
            url = ProductUrlGeneratorUtil
                    .getRemoteRegistryURLOfStratos(environment.getGreg().getProductVariables().getHttpsPort(),
                                                   environment.getGreg().getProductVariables().getHostName(),
                                                   frameworkProperties, userInfo);
        } else {
            url = ProductUrlGeneratorUtil
                    .getRemoteRegistryURLOfProducts(environment.getGreg().getProductVariables().getHttpsPort(),
                                                    environment.getGreg().getProductVariables().getHostName(),
                                                    environment.getGreg().getProductVariables()
                                                            .getWebContextRoot());
        }

        configurationBean.setRegistryURL(url);
        configurationBean.setUsername(UserListCsvReader.getUserInfo(2).getUserNameWithoutDomain());
        configurationBean.setPassword(UserListCsvReader.getUserInfo(2).getUserNameWithoutDomain());
    ...
    public void testScheduleReportNonExistingUser() throws Exception {
        ...
        configurationBean.setAttributes(testLCattributes);
        String url = ProductUrlGeneratorUtil
                .getRemoteRegistryURLOfProducts(environment.getGreg()
                                                        .getProductVariables().getHttpsPort(), environment
                        .getGreg().getProductVariables().getHostName(),
                                                environment.getGreg().getProductVariables()
                                                        .getWebContextRoot());
        configurationBean.setRegistryURL(url);
