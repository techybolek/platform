...
public class FilterServicesTestCase {
    ...
    public void testFilterEditedServices() throws GovernanceException {
        serviceForSearching2.addAttribute("test-att-service2", "test-val2");
        serviceManager.updateService(serviceForSearching2);
        Service searchResult = serviceManager.findServices(new ServiceFilter() {

            public boolean matches(Service service) throws GovernanceException {
                String attributeVal = service.getAttribute("test-att-service2");
                return attributeVal != null && attributeVal.startsWith("test-val2");
            }
        })[0];
    ...
    public void testFilterPromotedServices() throws GovernanceException, RemoteException,
        ...
        Service searchResult = serviceManager.findServices(new ServiceFilter() {
            public boolean matches(Service service) throws GovernanceException {
                String attributeVal = service.getAttribute("overview_name");
                String attributeVal2 = service.getAttribute("overview_version");
                return attributeVal != null && attributeVal.startsWith("serviceForPromotingNew") &&
     ...
     * @throws GovernanceException
     */
    @Test(groups = {"wso2.greg"}, description = "Filter Changed Services",
          dependsOnMethods = "testFilterPromotedServices")
     ...
     * @throws GovernanceException
     */
    @Test(groups = {"wso2.greg"}, description = "Seaching for a deleted service",
          dependsOnMethods = "testFilterChangedServices")
...
//                "<name>serviceForSea[A-Za-z1-9]+</name><version2>6.1.1</version2></overview>" +
//                "<serviceLifecycle /><contacts /><interface /><security /><endpoints /><docLinks />" +
//                "<operation xmlns=\"\">Add</operation><currentName xmlns=\"\"></currentName>" +
//                "<currentNamespace xmlns=\"\"></currentNamespace></serviceMetaData>";
//
//        Assert.assertEquals(listMetaDataServiceClient.listServices(criteria).getNames().length, 5, "Expect only 5 Search result");
//        Assert.assertEquals(listMetaDataServiceClient.listServices(criteria).getNames()[0],
//                            "serviceForSearching1", "Wrong Search result");
//    }
