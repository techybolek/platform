...
There are six samples in WSO2 CEP Server.
 ...
 5. Distributed Purchase Analyzer sample using Siddhi Engine in distributed mode, Agent Broker with Tuple mapping and JMS broker with XML Mapping
   ...
   1. Follow below steps to run StockQuote sample;
        ...
        Step 02: Run "ant deploy-xml" from wso2cep-3.0.0/samples directory to deploy required bucket.

        Step 03: Start the server.

        Step 04: Run "ant" from wso2cep-3.0.0/samples/services/FastMovingStockQuoteReceiverService directory to start the subscriber.
        ...
        Step 08: Run "ant xmlStockQuotePublisher" from wso2cep-3.0.0/samples directory to run the publisher to publish events to CEP.
   ...
   2. Follow below steps to run KPI Analyzer sample;
        ...
        Step 02: Run "ant deploy-agent" from wso2cep-3.0.0/samples directory to deploy required bucket.

        Step 03: Start the server.

        Step 04: Run "ant agentTestServer" in a separate terminal, from wso2cep-3.0.0/samples directory
                       to start the thrift output relieving server.

        Step 04: Run "ant agentPhoneRetailClient" in a separate terminal, from wso2cep-3.0.0/samples directory
   ...
   3. Follow below steps to run Twitter,StockQuote sample;
        ...
        Step 03: Copy paste activemq-all-xxx.jar from the ActiveMQ home directory to wso2cep-3.0.0/samples/lib directory

        Step 04: Copy paste activemq-core-xxx.jar and geronimo-j2ee-management_1.1_spec-1.0.1.jar from apache-activemq-xxx/lib
                to wso2cep-3.0.0/repository/components/lib directory

        Step 05: Run "ant deploy-jms" from wso2cep-3.0.0/samples directory to deploy required buckets.

        Step 06: Start the server.

        Step 07: Run "ant jmsSubscriber -Dtopic=PredictedStockQuotes" in a separate terminal, from wso2cep-3.0.0/samples directory
                to start the subscriber of "PredictedStockQuotes" topic.

        Step 08: Run "ant jmsAllStockQuotesPublisher" from wso2cep-3.0.0/samples directory
                to publish events to "AllStockQuotes" topic.

        Step 09: Run "ant jmsTwitterFeedPublisher" from wso2cep-3.0.0/samples directory
   ...
   4. Follow below steps to run Long Running Purchase Analyzer sample;
        ...
        Step 05: Run "ant deploy-persistence" from wso2cep-3.0.0/samples directory to deploy required buckets.

        Step 06: Start the server.

        Step 07: Run "ant jmsSubscriber -Dtopic=RetailSummary" in a separate terminal, from wso2cep-3.0.0/samples directory
                to start the subscriber of "RetailSummary" topic.

        Step 08: Run "ant agentPhoneRetailClient -Devents=5" from wso2cep-3.0.0/samples directory
                to publish events to CEP server.

        Step 09: Restart CEP

        Step 10: Run "ant agentPhoneRetailClient -Devents=1" from wso2cep-3.0.0/samples directory
                to publish events to CEP server.
   ...
   5. Follow below steps to run Distributed Purchase Analyzer sample;
        ...
        Step 05: Run "ant deploy-distributed" from wso2cep-3.0.0/samples directory to deploy required buckets.
        ...
        Step 09: Run "ant deploy-distributed" from wso2cep-3.0.0/samples directory to deploy required buckets.

        Step 10: Change the 2nd CEP server Offset from the file wso2cep-3.0.0-2/repository/conf/carbon.xml to 1 from 0, "e.g. <Offset>1</Offset>"

        Step 11: Start the 2nd server.

        Step 12: Run "ant jmsSubscriber -Dtopic=RetailSummary" in a separate terminal, from 1st server's wso2cep-3.0.0/samples directory
                to start the subscriber of "RetailSummary" topic.

        Step 13: Run "ant agentPhoneRetailClient -Dport=7611 -Devents=2" from the 1st server's wso2cep-3.0.0/samples directory
                to publish events to 1st CEP server.

        Step 14: Run "ant agentPhoneRetailClient -Dport=7612 -Devents=2" from the 1st server's wso2cep-3.0.0/samples directory
